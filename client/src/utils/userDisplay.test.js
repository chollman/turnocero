...
describe('getUserDisplay', () => {
  ...
  it('uses displayName when present', () => {
    const r = getUserDisplay({ _id: 'a', username: 'cha', displayName: 'Claudio Hollman' });
    expect(r.isDeleted).toBe(false);
    expect(r.name).toBe('Claudio Hollman');
  });

  it('falls back to nombre + apellido when displayName empty', () => {
    const r = getUserDisplay({ _id: 'a', username: 'cha', displayName: '', nombre: 'Claudio', apellido: 'Hollman' });
    expect(r.name).toBe('Claudio Hollman');
  });
  ...
  it('normalizes legacy string avatar to { url, publicId }', () => {
    const r = getUserDisplay({ _id: 'a', username: 'cha', avatar: 'https://x.com/y.jpg' });
  ...
  it('preserves username and displayName for downstream consumers (e.g. <Avatar>)', () => {
    const r = getUserDisplay({ _id: 'a', username: 'cha', displayName: 'Claudio H' });
