...
import { CheckIcon, ClockIcon, XIcon, EditIcon, TrashIcon, RefreshIcon } from './EventoIcons';
...
export default function TicketStub({
  ...
  const fillPct = hasMax ? Math.min(100, (participants / evento.maxParticipants) * 100) : 0;
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setLocalErr(err?.response?.data?.message || err?.message || 'No pudimos enviar tu inscripción.');
    }
  }
    ...
    <div className={styles.stub}>
        ...
        {d && (
          <div className={styles.dateBlock}>
            ...
            <div className={styles.dateRight}>
              ...
              <div className={styles.weekday}>{d.weekdayLong} · {d.year}</div>
            </div>
          </div>
        )}
        {d && <div className={styles.time}>⏱ {d.time} hs (hora local)</div>}
        {!isPast && evento.status === 'open' && cd.text && (
          <div className={`${styles.countdown} ${styles[`countdown_${cd.tone}`] || ''}`}>
            ● {cd.text}
          </div>
        )}
        {isPast && cd.text && (
      ...
      <div className={styles.bottom}>
        <div className={styles.rowItem}>
          ...
          <span className={`${styles.rowValue} ${isFree ? styles.rowValueFree : ''}`}>{formatFee(evento.fee)}</span>
        </div>
        <div className={styles.rowItem}>
          <span className={styles.rowLabel}>Cupo</span>
          <span className={styles.rowValue}>
            {hasMax ? (
              <>{participants}<span className={styles.cupoTotal}>/{evento.maxParticipants}</span></>
            ) : (
        ...
        {hasMax && (
          <div className={styles.cuposBar}>
            <div className={`${styles.cuposFill} ${isFull ? styles.cuposFillFull : ''}`} style={{ width: `${fillPct}%` }} />
          </div>
        )}
        <div className={styles.rowItem}>
        ...
        <div className={styles.ctaBlock}>
          {isHost ? (
            <>
              {onOpenInscripciones && (
                <button className={styles.cta} onClick={onOpenInscripciones} type="button">
              ...
              <div className={styles.adminBlock}>
                ...
                <div className={styles.adminActions}>
                  {onEdit && (
                    <button className={styles.adminBtn} type="button" onClick={onEdit}>
                      <EditIcon size={11} />&nbsp;Editar
                    </button>
                  )}
                  {evento.status === 'cancelled' && onReopen ? (
                    <button className={styles.adminBtn} type="button" onClick={onReopen}>
                      <RefreshIcon size={11} />&nbsp;Reabrir
                    </button>
                  ) : (onDelete || onCancelEvent) && !confirmDelete ? (
                    <button className={styles.adminBtn} type="button" onClick={() => setConfirmDelete(true)}>
                      <TrashIcon size={11} />&nbsp;Cancelar
                    </button>
                  ) : null}
                </div>
                {confirmDelete && (
                  <div className={styles.confirmRow}>
                    <span className={styles.confirmText}>¿Cancelar este evento?</span>
                    <button
                      className={`${styles.adminBtn} ${styles.adminBtnConfirm}`}
                      onClick={() => { setConfirmDelete(false); (onDelete || onCancelEvent)(); }}
                      type="button"
                    >
                      Sí
                    </button>
            ...
            <button className={styles.cta} type="button" onClick={onLoginRequest}>
              Iniciá sesión para inscribirte
            </button>
          ) : status === 'confirmed' ? (
            <div className={`${styles.state} ${styles.stateConfirmed}`}>
              <CheckIcon size={20} />
              <span className={styles.stateTitle}>¡Inscripción confirmada!</span>
            ...
            <>
              <div className={`${styles.state} ${styles.statePending}`}>
                ...
                <span className={styles.stateSub}>El admin revisará tu comprobante</span>
              </div>
              {userRegistration?.comprobante?.url && (
                <a href={userRegistration.comprobante.url} target="_blank" rel="noopener noreferrer" className={styles.ghostBtn}>
                  Ver comprobante enviado
                </a>
              )}
              {!confirmCancel ? (
                <button className={styles.ghostBtn} type="button" onClick={() => setConfirmCancel(true)}>
                  Cancelar inscripción
                </button>
              ) : (
                <div className={styles.confirmRow}>
                  <span className={styles.confirmText}>¿Cancelar tu inscripción?</span>
                  <button
                    className={`${styles.adminBtn} ${styles.adminBtnConfirm}`}
                    type="button"
                    onClick={async () => { await onCancelRegistration(); setConfirmCancel(false); }}
                    disabled={cancellingReg}
                  >
                    {cancellingReg ? '…' : 'Sí'}
                  </button>
                  <button className={styles.adminBtn} type="button" onClick={() => setConfirmCancel(false)}>
                    No
                  </button>
                </div>
              )}
            ...
            <button className={styles.ghostBtn} disabled type="button">Inscripciones cerradas</button>
          ) : evento.status === 'draft' ? (
            <button className={styles.ghostBtn} disabled type="button">Aún no publicado</button>
          ) : isFull ? (
            <button className={styles.ghostBtn} disabled type="button">Sin cupo disponible</button>
          ) : showForm ? (
            <div className={styles.form}>
              ...
              {!isFree && evento.transferDetails && (
                <div>
                  <div className={styles.formLabel}>◆ Datos de transferencia · {formatFee(evento.fee)}</div>
              ...
              <div className={styles.formActions}>
                <button
                  ...
                  type="button"
                  onClick={() => { setShowForm(false); setComprobante(null); setLocalErr(''); }}
                  disabled={inscribing}
                >
                  Cancelar
                </button>
            ...
            <button className={styles.cta} type="button" onClick={() => setShowForm(true)}>
