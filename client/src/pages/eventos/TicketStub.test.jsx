...
describe('<TicketStub>', () => {
  it('renders date block and fee', () => {
    render(<TicketStub evento={makeEvento()} user={{ _id: 'u1' }} onInscribirse={noop} />);
  ...
  it('shows login button when user is not authenticated', () => {
    const onLogin = vi.fn();
    render(<TicketStub evento={makeEvento()} user={null} onLoginRequest={onLogin} onInscribirse={noop} />);
  ...
  it('skips comprobante for free events', () => {
    render(<TicketStub evento={makeEvento({ fee: 0 })} user={{ _id: 'u1' }} onInscribirse={noop} />);
  ...
  it('shows host admin actions when isHost is true', () => {
    ...
    const manage = screen.getByRole('button', { name: /gestionar inscripciones/i });
  ...
  it('shows "Inscripciones cerradas" for closed events when user not registered', () => {
    ...
    expect(screen.getByRole('button', { name: /inscripciones cerradas/i })).toBeDisabled();
  });
  ...
  it('shows "Evento cancelado" for cancelled events when user not registered', () => {
    ...
    expect(screen.getByRole('button', { name: /evento cancelado/i })).toBeDisabled();
  });

  it('disables CTA when event is full', () => {
    render(
      <TicketStub
        evento={makeEvento({ registrationCount: { total: 24, pending: 0, confirmed: 24 } })}
        user={{ _id: 'u1' }}
        onInscribirse={noop}
      />
    );
  ...
  it('calls onInscribirse with comprobante when submitting paid event form', async () => {
    ...
    render(<TicketStub evento={makeEvento()} user={{ _id: 'u1' }} onInscribirse={onInscribirse} />);
    fireEvent.click(screen.getByRole('button', { name: /inscribirme/i }));
