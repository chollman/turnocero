...
describe("<ImageDropzone>", () => {
  it("renders placeholder when there is no preview", () => {
    ...
    expect(
      screen.getByText(/arrastrá, pegá o hacé click/i),
    ).toBeInTheDocument();
  ...
  it("renders image when preview prop is provided", () => {
    render(
      <ImageDropzone preview="https://example.com/x.jpg" onFile={() => {}} />,
    );
  ...
  it("calls onFile when a file is dropped", () => {
    ...
    fireEvent.drop(dz, {
      dataTransfer: { files: [makeFile("drop.webp", "image/webp")] },
    });
    expect(onFile).toHaveBeenCalledTimes(1);
  });
