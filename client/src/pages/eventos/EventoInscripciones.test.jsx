...
function makeReg(id, status, overrides = {}) {
  return {
    _id: `r${id}`,
    user: { _id: id, username: `u${id}`, displayName: `User ${id}`, avatar: null },
    status,
...
function setupResponse(payload) {
  server.use(http.get('/api/eventos/:id/inscripciones', () => HttpResponse.json(payload)));
}
...
beforeEach(() => {
  setupResponse({
    ...
    evento: { _id: 'e1', title: 'Torneo Catán', eventDate: '2026-06-13T17:00:00', maxParticipants: 24 },
  });
});
...
describe('<EventoInscripciones>', () => {
  ...
  it('accepting calls the confirmar endpoint with the right user id', async () => {
    ...
    server.use(
      http.patch('/api/eventos/:id/inscripciones/:userId/confirmar', ({ params }) => {
  ...
  it('rejecting calls the rechazar endpoint with the right user id', async () => {
    ...
    server.use(
      http.patch('/api/eventos/:id/inscripciones/:userId/rechazar', ({ params }) => {
