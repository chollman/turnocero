...
describe("<UsersList>", () => {
  ...
  it("regular users do not see ban/delete buttons", async () => {
    ...
    expect(
      screen.queryByRole("button", { name: /banear/i }),
  ...
  it("user with displayName shows it below the username", async () => {
    setup({
      users: [
        makeUserCard({ username: "alice123", displayName: "Alice García" }),
      ],
    });
  ...
  it("shows filter toggle buttons (Solo activos / Solo amigos / Con BG Watch)", async () => {
    ...
    await screen.findByText("@alice");
    expect(
      screen.getByRole("button", { name: /solo activos/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /solo amigos/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /con bg watch/i }),
    ).toBeInTheDocument();
  });
  ...
  it("shows sort select with all sort options", async () => {
    ...
    expect(
      screen.getByRole("option", { name: "Más activos" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("option", { name: "Más nuevos" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("option", { name: "Más antiguos" }),
    ).toBeInTheDocument();
  });
