...
beforeEach(() => {
  server.use(
    http.get('/api/bgg/partidas/:bggUsername', () =>
      HttpResponse.json({ plays: [{ id: 'p1', date: '2026-05-01' }], total: 30 }),
    ),
    http.get('/api/bgg/coleccion/:bggUsername', () =>
      HttpResponse.json([{ id: 13, name: 'Catán' }, { id: 14, name: 'Azul' }]),
    ),
  );
});
...
describe('<MiBgWatchCard>', () => {
  ...
  it('renders error note when both APIs fail', async () => {
    ...
    renderCard();
    await waitFor(() => {
      expect(screen.getByText(/igual podés entrar a tu bg watch/i)).toBeInTheDocument();
    });
  });
