...
describe('<TableDetail>', () => {
  ...
  it('host sees HOST badge and "Editar mesa" + "Cancelar mesa" buttons', async () => {
    setupTable(makeTable({
      host: { _id: 'host1', username: 'host1', avatar: { url: '', publicId: '' } },
    }));
    renderTableDetail({ user: { _id: 'host1', username: 'host1', isAdmin: false } });
    await screen.findByText('Catán');
    expect(screen.getByText('HOST')).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /editar mesa/i })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /cancelar mesa/i })).toBeInTheDocument();
  });

  it('player sees UNIDO badge and "Abandonar mesa" button', async () => {
    setupTable(makeTable({
      players: [{ _id: 'me', username: 'me', avatar: { url: '', publicId: '' } }],
    }));
    renderTableDetail({ user: { _id: 'me', username: 'me', isAdmin: false } });
    await screen.findByText('Catán');
    expect(screen.getByText('UNIDO')).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /abandonar mesa/i })).toBeInTheDocument();
  });

  it('guest sees "Unirme a la mesa" and "🔕 Seguir" buttons', async () => {
    setupTable(makeTable());
    renderTableDetail({ user: { _id: 'other', username: 'other', isAdmin: false } });
    await screen.findByText('Catán');
  ...
  it('full open table shows "Mesa completa" on the join button', async () => {
    ...
    renderTableDetail({ user: { _id: 'other', username: 'other', isAdmin: false } });
    await screen.findByText('Catán');
    expect(screen.getByRole('button', { name: /mesa completa/i })).toBeDisabled();
  });
  ...
  it('cancelled table shows CANCELADA badge and hides action bar', async () => {
    ...
    expect(screen.queryByRole('button', { name: /unirme a la mesa/i })).not.toBeInTheDocument();
  });

  it('admin viewing a table they are not part of sees the admin banner', async () => {
    setupTable(makeTable());
    renderTableDetail({ user: { _id: 'admin1', username: 'admin1', isAdmin: true } });
    await screen.findByText('Catán');
    expect(screen.getByText(/viendo esta mesa como administrador/i)).toBeInTheDocument();
  });
  ...
  it('shows the seat count and "lugares libres" status text', async () => {
    setupTable(makeTable({
      maxPlayers: 4,
      players: [{ _id: 'a', username: 'a', avatar: { url: '', publicId: '' } }],
    }));
    renderTableDetail();
    await screen.findByText('Catán');
  ...
  it('host on a private table with pending requests sees Aceptar/Rechazar buttons', async () => {
    setupTable(makeTable({
      ...
      host: { _id: 'host1', username: 'host1', avatar: { url: '', publicId: '' } },
      pendingRequests: [
        { _id: 'r1', username: 'requester1', avatar: { url: '', publicId: '' } },
      ],
    }));
    renderTableDetail({ user: { _id: 'host1', username: 'host1', isAdmin: false } });
    await screen.findByText('Catán');
  ...
  it('shows the empty comments state when no comments exist', async () => {
    ...
    renderTableDetail({ user: { _id: 'me', username: 'me', isAdmin: false } });
    await screen.findByText('Catán');
    expect(screen.getByText(/nadie coment[oó] todav[ií]a/i)).toBeInTheDocument();
  });
  ...
  it('renders a list of comments when comments exist', async () => {
        ...
        HttpResponse.json([
          {
            ...
            author: { _id: 'a1', username: 'commenter1', avatar: { url: '', publicId: '' } },
    ...
    renderTableDetail({ user: { _id: 'me', username: 'me', isAdmin: false } });
    await screen.findByText('Catán');
    await waitFor(() => expect(screen.getByText('Excelente partida!')).toBeInTheDocument());
  });
  ...
  it('renders gallery images when images exist', async () => {
    setupTable(makeTable({
      images: [
        { _id: 'i1', url: 'https://example.com/photo.jpg', uploader: { _id: 'u1', username: 'u1' } },
      ],
    }));
    renderTableDetail({ user: { _id: 'me', username: 'me', isAdmin: false } });
    await screen.findByText('Catán');
    ...
    server.use(
      http.post('/api/tables/:id/join', () => {
        ...
        return HttpResponse.json({
          table: { ...makeTable(), players: [{ _id: 'other', username: 'other' }] },
        });
      }),
    );
    renderTableDetail({ user: { _id: 'other', username: 'other', isAdmin: false } });
  ...
  it('clicking a reaction emoji triggers POST /react', async () => {
    ...
    server.use(
      http.post('/api/tables/:id/react', async ({ request }) => {
        ...
        reacted(body);
        return HttpResponse.json({ reactions: [{ user: 'me', emoji: body.emoji }] });
      }),
    );
    renderTableDetail({ user: { _id: 'me', username: 'me', isAdmin: false } });
    await screen.findByText('Catán');
  ...
  it('guest with pending request clicking "Cancelar" calls DELETE /request', async () => {
    ...
    server.use(
      http.delete('/api/tables/:id/request', () => {
        cancelled();
        return HttpResponse.json({ table: { ...makeTable(), pendingRequests: [] } });
      }),
    );
    renderTableDetail({ user: { _id: 'me', username: 'me', isAdmin: false } });
    await screen.findByText('Catán');
  ...
  it('host clicking accept on a pending request calls POST /requests/:userId/accept', async () => {
    ...
    server.use(
      http.post('/api/tables/:id/requests/:userId/accept', () => {
        accepted();
        return HttpResponse.json({
          ...makeTable({ privacy: 'private', host: { _id: 'host1', username: 'host1' } }),
          pendingRequests: [],
        });
      }),
    );
