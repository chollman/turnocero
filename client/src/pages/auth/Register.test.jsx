...
function fillValidForm({ password = 'Password123', confirm = 'Password123' } = {}) {
  fireEvent.change(screen.getByPlaceholderText('tu_nombre_de_jugador'), { target: { value: 'newcomer' } });
  fireEvent.change(screen.getByPlaceholderText('tu@email.com'), { target: { value: 'new@b.com' } });
  fireEvent.change(screen.getByPlaceholderText('Mín. 8 caracteres, 1 mayúscula y 1 número'), {
...
describe('<Register>', () => {
  ...
  it('calls register(username, email, password) on valid submit and navigates to /verificar-email', async () => {
    ...
    await waitFor(() => {
      expect(register).toHaveBeenCalledWith('newcomer', 'new@b.com', 'Password123');
    });
    expect(navigateMock).toHaveBeenCalledWith(
      '/verificar-email',
      expect.objectContaining({ state: expect.objectContaining({ email: 'new@b.com' }) }),
    );
  });
