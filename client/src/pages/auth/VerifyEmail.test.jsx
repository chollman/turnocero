...
describe("<VerifyEmail>", () => {
  ...
  it("on success calls verifyEmail(email, code), clears sessionStorage, and navigates to /", async () => {
    ...
    await waitFor(() =>
      expect(verifyEmail).toHaveBeenCalledWith("pending@b.com", "555555"),
    );
  ...
  it("Resend button triggers requestEmailVerification and starts a cooldown", async () => {
    ...
    const resendBtn = screen.getByRole("button", {
      name: /reenviar c[oó]digo/i,
    });
    fireEvent.click(resendBtn);

    await waitFor(() =>
      expect(requestEmailVerification).toHaveBeenCalledWith("a@b.com"),
    );
