...
describe("BG Watch guest CTAs", () => {
  ...
  it("GuestInlineCTA renders the soft-sell copy", () => {
    ...
    expect(
      screen.getByText(/¿Tenés cuenta en BoardGameGeek\?/i),
    ).toBeInTheDocument();
    expect(screen.getByRole("link", { name: /empezar/i })).toHaveAttribute(
      "href",
      "/register?source=bg-watch",
    );
  });
  ...
  it("GuestFooter renders the bggUsername and final CTA", () => {
    ...
    ).toHaveAttribute("href", "/register?source=bg-watch");
  });
});
