...
vi.mock("./components/TorneoCard", () => ({
  default: ({ torneo }) => (
    <div data-testid="torneo-card" data-status={torneo.status}>
      {torneo.title}
    </div>
  ),
}));
...
describe("<Torneos>", () => {
  ...
  it('admin in admin mode sees the "Nuevo torneo" button', async () => {
    ...
    expect(
      screen.getByRole("button", { name: /nuevo torneo|crear torneo/i }),
    ).toBeInTheDocument();
  });
  ...
  it('admin viewing-as-user does NOT see the "Nuevo torneo" button', async () => {
    ...
    await screen.findByText("Open Tournament");
    expect(
      screen.queryByRole("button", { name: /nuevo torneo|crear torneo/i }),
