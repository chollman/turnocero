...
function renderBtn({ torneo = makeTorneo(), user = null, onChange = vi.fn() } = {}) {
  setupNotifs();
  return render(
    <MemoryRouter>
...
describe('<RegisterButton>', () => {
  it('renders nothing when torneo is not in registration', () => {
    const { container } = renderBtn({ torneo: makeTorneo({ status: 'in_progress' }) });
    expect(container.firstChild).toBeNull();
  });

  it('renders nothing for admin_only inscriptionMode', () => {
    const { container } = renderBtn({ torneo: makeTorneo({ inscriptionMode: 'admin_only' }), user: { _id: 'me' } });
    expect(container.firstChild).toBeNull();
  });

  it('shows the login CTA for anonymous users', () => {
    renderBtn({ user: null });
    expect(screen.getByRole('button', { name: /inici[aá] sesi[oó]n/i })).toBeInTheDocument();
  });
  ...
  it('shows "Cupo lleno" disabled state when maxParticipants is reached', () => {
    renderBtn({
      torneo: makeTorneo({ participants: [{ _id: 'p1' }, { _id: 'p2' }], maxParticipants: 2 }),
      user: { _id: 'me' },
    });
  ...
  it('clicking Inscribirme posts to /register and calls onChange', async () => {
    ...
    server.use(http.post('/api/torneos/:id/register', () => HttpResponse.json({ ok: true })));
  ...
  it('clicking Cancelar deletes /register and calls onChange', async () => {
    const onChange = vi.fn();
    server.use(http.delete('/api/torneos/:id/register', () => HttpResponse.json({ ok: true })));
    renderBtn({
      torneo: makeTorneo({ pendingRegistrations: [{ user: { _id: 'me' } }] }),
      user: { _id: 'me' },
      onChange,
  ...
  it('shows API error on register failure', async () => {
    server.use(
      http.post('/api/torneos/:id/register', () => HttpResponse.json({ message: 'Cupo ya lleno' }, { status: 400 })),
    );
    renderBtn({ user: { _id: 'me' } });
    fireEvent.click(screen.getByRole('button', { name: /inscribirme/i }));
