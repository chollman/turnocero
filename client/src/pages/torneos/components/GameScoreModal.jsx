...
export default function GameScoreModal({ game, players, onClose, onConfirm }) {
  ...
  useEffect(() => {
    ...
    if (game?.results?.length) {
      for (const r of game.results) initial[String(r.player?._id || r.player)] = String(r.score)
    }
    setScores(initial)
    setError('')
  ...
  const positions = useMemo(() => {
    ...
    valid.forEach((e, idx) => {
      if (e.score !== last) { pos = idx + 1; last = e.score }
      posMap[e.id] = pos
    })
    return posMap
  }, [scores, players])
    ...
    <div className={styles.modalBackdrop} onClick={onClose}>
      <div className={styles.modalCard} onClick={(e) => e.stopPropagation()}>
        <div className={styles.modalHeader}>
          <h3 className={styles.modalTitle}>Cargar resultado · Partida {game.gameNumber}</h3>
          <button className={styles.modalClose} onClick={onClose} aria-label="Cerrar">✕</button>
        </div>
        <p className={styles.modalSub}>Ingresá el puntaje real de cada jugador. La posición se calcula automáticamente.</p>
          ...
          {(players || []).map((p) => {
            ...
            return (
              <li key={id} className={styles.scoreItem}>
                <span className={styles.scorePos}>{pos ? `${pos}º` : '—'}</span>
