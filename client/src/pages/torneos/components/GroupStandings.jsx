...
export default function GroupStandings({ standings, players, gamesPerGroup, qualifiersPerGroup }) {
      ...
      <table className={styles.standingsTable}>
        <thead>
          <tr>
            ...
            {Array.from({ length: gamesPerGroup }).map((_, i) => (
              <th key={i} className={styles.colNum}>P{i + 1}</th>
            ))}
        ...
        <tbody>
          {standings.map((row, idx) => {
            ...
            return (
              <tr key={row.user} className={isQualifying ? styles.rowLeader : ''}>
                ...
                <td className={`${styles.colNum} ${styles.colPts}`}>{row.totalPV}</td>
              </tr>
            )
          })}
        </tbody>
