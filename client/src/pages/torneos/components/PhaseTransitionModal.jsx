...
export default function PhaseTransitionModal({
  ...
  useEffect(() => {
    axios
      ...
      .catch((err) =>
        setError(
          err.response?.data?.message || "Error al calcular la vista previa",
        ),
      )
  ...
  const submit = async (tableSize) => {
    ...
    try {
      ...
      setError(
        err.response?.data?.message || "Error al generar la siguiente fase",
      );
    } finally {
      setSub(false);
    }
              ...
              {preview.valid ? (
                preview.isFinal ? (
                  <p className={styles.phasePreviewNote}>
                    🏁 Esta será la <strong>mesa final</strong> con{" "}
                    {preview.finalTableSize} jugadores.
                  </p>
                ) : (
                  <p className={styles.phasePreviewNote}>
                    Se generarán{" "}
                    <strong>
                      {preview.tables} mesas de {preview.finalTableSize}
                    </strong>
                    .
                  </p>
                )
