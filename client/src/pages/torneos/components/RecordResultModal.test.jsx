...
const PLAYER_A = { _id: 'a', username: 'alice', avatar: { url: '', publicId: '' } };
const PLAYER_B = { _id: 'b', username: 'bob', avatar: { url: '', publicId: '' } };

function renderModal({ match, format = 'single_elim', onConfirm = vi.fn(), onClose = vi.fn() } = {}) {
  return render(
    <MemoryRouter>
      <RecordResultModal match={match} format={format} onConfirm={onConfirm} onClose={onClose} />
    </MemoryRouter>,
  );
}
...
describe('<RecordResultModal>', () => {
  ...
  it('does NOT show "Empate" option for single_elim format', () => {
    renderModal({ match: { _id: 'm1', playerA: PLAYER_A, playerB: PLAYER_B }, format: 'single_elim' });
    expect(screen.queryByText(/empate/i)).not.toBeInTheDocument();
  });

  it('shows "Empate" option for league format', () => {
    renderModal({ match: { _id: 'm1', playerA: PLAYER_A, playerB: PLAYER_B }, format: 'league' });
  ...
  it('selecting a winner enables Confirm + calls onConfirm with { winnerId }', async () => {
    ...
    renderModal({ match: { _id: 'm1', playerA: PLAYER_A, playerB: PLAYER_B }, onConfirm });
    // Click "alice"
    fireEvent.click(screen.getByText('alice').closest('button'));
    const confirm = screen.getByRole('button', { name: 'Confirmar' });
    expect(confirm).not.toBeDisabled();
    fireEvent.click(confirm);
    await waitFor(() => expect(onConfirm).toHaveBeenCalledWith({ winnerId: 'a' }));
  });

  it('selecting Empate (league) calls onConfirm with { draw: true }', async () => {
    const onConfirm = vi.fn().mockResolvedValue(undefined);
    renderModal({ match: { _id: 'm1', playerA: PLAYER_A, playerB: PLAYER_B }, format: 'league', onConfirm });
  ...
  it('Cancel button calls onClose', () => {
    const onClose = vi.fn();
    renderModal({ match: { _id: 'm1', playerA: PLAYER_A, playerB: PLAYER_B }, onClose });
