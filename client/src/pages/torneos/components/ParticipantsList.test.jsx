...
function renderList({ torneo = makeTorneo(), isAdmin = true, onChange = vi.fn() } = {}) {
  return render(
    <MemoryRouter>
...
describe('<ParticipantsList>', () => {
  it('renders an empty state when there are no participants', () => {
    ...
    expect(screen.getByText(/no hay participantes inscriptos/i)).toBeInTheDocument();
  });
  ...
  it('admin in in_progress phase does NOT see "Quitar" (no more removals after start)', () => {
    renderList({ isAdmin: true, torneo: makeTorneo({ status: 'in_progress' }) });
  ...
  it('clicking Quitar shows Sí/No confirm, Sí calls DELETE + onChange', async () => {
    ...
    server.use(http.delete('/api/torneos/:id/participants/:userId', () =>
