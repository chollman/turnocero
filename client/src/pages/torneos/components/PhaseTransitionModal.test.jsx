...
function setupPreview(preview) {
  server.use(http.get('/api/torneos/:id/next-phase/preview', () => HttpResponse.json(preview)));
}

function renderModal({ onClose = vi.fn(), onGenerated = vi.fn() } = {}) {
  return render(
    <MemoryRouter>
      <PhaseTransitionModal torneoId="t1" onClose={onClose} onGenerated={onGenerated} />
    </MemoryRouter>,
  );
}
...
describe('<PhaseTransitionModal>', () => {
  it('shows the heading once preview loads', async () => {
    setupPreview({ advancersCount: 4, defaultTableSize: 4, valid: true, isFinal: true, finalTableSize: 4 });
    renderModal();
    expect(await screen.findByText(/generar siguiente fase/i)).toBeInTheDocument();
  });
  ...
  it('renders the "N mesas de M" note when valid but not final', async () => {
    setupPreview({ advancersCount: 8, defaultTableSize: 4, valid: true, isFinal: false, finalTableSize: 4, tables: 2 });
    renderModal();
  ...
  it('renders suggestion buttons when preview is invalid', async () => {
    ...
    expect(screen.getByRole('button', { name: 'Una sola mesa de 5' })).toBeInTheDocument();
  });
  ...
  it('clicking a suggestion marks it active', async () => {
    ...
    renderModal();
    const btn = await screen.findByRole('button', { name: 'Una sola mesa de 5' });
    fireEvent.click(btn);
  ...
  it('clicking Generar (valid preview) POSTs to /next-phase and calls onGenerated', async () => {
    ...
    server.use(http.post('/api/torneos/:id/next-phase', () => HttpResponse.json({ ok: true })));
