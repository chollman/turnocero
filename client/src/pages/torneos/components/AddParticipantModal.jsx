    ...
    <div className={styles.modalBackdrop} onClick={onClose}>
      <div className={`${styles.modalCard} ${styles.modalCardWide}`} onClick={(e) => e.stopPropagation()}>
        ...
        <p className={styles.modalSub}>
          Buscá por nombre o usuario. {cupoLleno && '⚠ El torneo alcanzó el cupo máximo.'}
        </p>
              ...
              return (
                <li key={id} className={styles.userPickItem}>
                  <span className={styles.userPickName}>
                    ...
                    {u.username && <span className={styles.userPickHandle}>@{u.username}</span>}
                  </span>
                  {already ? (
                    <span className={styles.userPickAdded}>✓ Inscripto</span>
                  ) : pending ? (
                    <button className={styles.userPickAddBtn} onClick={() => addUser(id)} disabled={!!adding || cupoLleno}>
