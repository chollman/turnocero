...
export default function LeagueRoundsList({
  matches,
  isAdmin,
  onRecord,
  onUndo,
...
function MatchRow({ match, isAdmin, onRecord, onUndo }) {
  ...
  return (
    <div
      className={`${styles.matchRow} ${isCompleted ? styles.matchRowDone : ""} ${isBye ? styles.matchRowBye : ""}`}
    >
      <div
        className={`${styles.matchSide} ${winId === aId ? styles.matchSideWinner : ""}`}
      >
        {match.playerA ? (
