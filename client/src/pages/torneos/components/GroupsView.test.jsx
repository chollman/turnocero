...
vi.mock("./GroupStandings", () => ({
  default: ({ standings }) => (
    <div data-testid="group-standings">
...
function setupGroups(data) {
  server.use(
    http.get("/api/torneos/:id/groups", () => HttpResponse.json(data)),
  );
}
...
describe("<GroupsView>", () => {
      ...
      groups: [
        {
          ...
          advancedPlayers: [],
          games: [
            { _id: "gm1", gameNumber: 1, status: "pending", results: [] },
          ],
          standings: [],
        },
      ],
  ...
  it('completed game shows "✓ Cargada" and admin sees "Deshacer"', async () => {
    ...
    renderView({ isAdmin: true });
    await waitFor(() =>
      expect(screen.getByText(/✓ cargada/i)).toBeInTheDocument(),
    );
    expect(
  ...
  it("multiple phases render phase tabs", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole("button", { name: /fase 1/i }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("button", { name: /fase 2/i }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("button", { name: /fase 3/i }),
      ).toBeInTheDocument();
    });
  });
  ...
  it('admin can click "Editar" to enter advanced-players edit mode (shows checkboxes)', async () => {
    ...
    expect(
      screen.getByRole("button", { name: /^listo$/i }),
    ).toBeInTheDocument();
  });
  ...
  it('admin sees "Generar siguiente fase" button when all groups completed with advanced players', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole("button", { name: /generar siguiente fase/i }),
      ).toBeInTheDocument();
    });
  });
});
