...
export default function ParticipantsList({ torneo, isAdmin, onChange }) {
  ...
  const canRemove = isAdmin && (torneo.status === 'draft' || torneo.status === 'registration')

  if (participants.length === 0) {
    return <p className={styles.emptyMsg}>Todavía no hay participantes inscriptos.</p>
  }
  ...
  return (
    <li className={styles.partItem}>
      ...
      <span className={styles.partUser}>
        {info.isDeleted ? <span className={styles.deletedTxt}>Usuario eliminado</span> : <UserRef user={user} />}
      </span>
      {canRemove && !info.isDeleted && (
        <span className={styles.partActions}>
          {confirming ? (
            <>
              <button className={styles.confirmYes} onClick={remove} disabled={busy}>Sí</button>
              <button className={styles.confirmNo} onClick={() => setConfirm(false)}>No</button>
            </>
          ) : (
            <button className={styles.partRemove} onClick={() => setConfirm(true)}>Quitar</button>
          )}
        </span>
      )}
    </li>
