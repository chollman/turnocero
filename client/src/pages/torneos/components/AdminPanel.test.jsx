...
describe("<AdminPanel>", () => {
  ...
  it('draft → shows "Abrir inscripciones" as the next action', () => {
    ...
    expect(
      screen.getByRole("button", { name: /abrir inscripciones/i }),
    ).toBeInTheDocument();
  });
  ...
  it('registration → shows "Iniciar torneo" and "Volver a borrador"', () => {
    ...
    expect(
      screen.getByRole("button", { name: /iniciar torneo/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /volver a borrador/i }),
    ).toBeInTheDocument();
  });
  ...
  it('in_progress → shows "Finalizar torneo" + Reset button', () => {
    ...
    expect(
      screen.getByRole("button", { name: /finalizar torneo/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /reiniciar torneo/i }),
    ).toBeInTheDocument();
  });
  ...
  it('finished → no "next" button + still allows reset', () => {
    ...
    expect(
      screen.queryByRole("button", { name: /abrir inscripciones/i }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: /^iniciar torneo$/i }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: /^finalizar torneo$/i }),
    ).not.toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /reiniciar torneo/i }),
    ).toBeInTheDocument();
  ...
  it('admin_only inscription mode + draft → shows "+ Agregar participantes"', () => {
    renderPanel({
      torneo: makeTorneo({ status: "draft", inscriptionMode: "admin_only" }),
    });
    expect(
      screen.getByRole("button", { name: /agregar participantes/i }),
    ).toBeInTheDocument();
  });

  it('"Reordenar seeds" is disabled when fewer than 2 participants', () => {
    renderPanel({
      torneo: makeTorneo({ status: "draft", participants: [{ _id: "p1" }] }),
    });
    expect(
      screen.getByRole("button", { name: /reordenar seeds/i }),
    ).toBeDisabled();
  });
  ...
  it("Eliminar: confirm flow Sí calls DELETE + onDelete", async () => {
    ...
    server.use(
      http.delete("/api/torneos/:id", () => HttpResponse.json({ ok: true })),
    );
  ...
  it('"Finalizar torneo" shows confirmation inline', async () => {
    ...
    expect(
      screen.getByRole("button", { name: /sí, finalizar/i }),
    ).toBeInTheDocument();
  });
  ...
  it('"Finalizar torneo": clicking Cancelar dismisses confirmation', async () => {
    ...
    expect(
      screen.queryByText(/¿finalizar el torneo\?/i),
    ).not.toBeInTheDocument();
  });
  ...
  it('"↻ Reiniciar torneo": clicking shows reset confirmation', async () => {
    ...
    expect(
      screen.getByText(/se borrarán todas las partidas/i),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /sí, reiniciar/i }),
    ).toBeInTheDocument();
  });
  ...
  it('"↻ Reiniciar torneo": Cancelar hides confirmation', async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: /^cancelar$/i }));
    expect(
      screen.queryByText(/se borrarán todas las partidas/i),
    ).not.toBeInTheDocument();
  });
  ...
  it('"↻ Reiniciar torneo": Sí, reiniciar calls POST /reset and onChange', async () => {
    const onChange = vi.fn();
    server.use(
      http.post("/api/torneos/:id/reset", () =>
        HttpResponse.json({ status: "in_progress" }),
      ),
    );
    renderPanel({ torneo: makeTorneo({ status: "in_progress" }), onChange });
    fireEvent.click(screen.getByRole("button", { name: /reiniciar torneo/i }));
  ...
  it("PATCH /status error shows error message", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText(/no se puede iniciar/i)).toBeInTheDocument(),
    );
  });
});
