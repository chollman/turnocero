...
describe("<CreateTorneo>", () => {
  ...
  it("successful submit POSTs to /api/torneos and navigates to detail", async () => {
    ...
    if (textboxes[2])
      fireEvent.change(textboxes[2], { target: { value: "Catán" } });
  ...
  it("shows API error when create fails", async () => {
    server.use(
      http.post("/api/torneos", () =>
        HttpResponse.json(
          { message: "Solo admins pueden crear torneos" },
          { status: 403 },
        ),
      ),
    );
    ...
    if (textboxes[2])
      fireEvent.change(textboxes[2], { target: { value: "G" } });
