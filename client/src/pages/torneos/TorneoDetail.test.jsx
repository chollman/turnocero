...
vi.mock("./components/RegisterButton", () => ({
  default: () => <button>Inscribirme</button>,
}));
vi.mock("./components/LeagueStandings", () => ({
  default: () => <div data-testid="league-standings" />,
}));
vi.mock("./components/LeagueRoundsList", () => ({
  default: () => <div data-testid="rounds-list" />,
}));
vi.mock("./components/Bracket", () => ({
  default: () => <div data-testid="bracket" />,
}));
vi.mock("./components/RecordResultModal", () => ({ default: () => null }));
vi.mock("./components/SeedReorderModal", () => ({
  default: () => <div data-testid="seed-reorder-modal" />,
}));
vi.mock("./components/AddParticipantModal", () => ({
  default: () => <div data-testid="add-participant-modal" />,
}));
vi.mock("./components/GroupsView", () => ({
  default: () => <div data-testid="groups-view" />,
}));
...
describe("<TorneoDetail>", () => {
  ...
  it("renders the league format label + standings tab (default for league)", async () => {
    ...
    expect(
      screen.getByText(/Liga \(todos contra todos\)/i),
    ).toBeInTheDocument();
  });
  ...
  it("admin clicking add participants shows AddParticipantModal", async () => {
    ...
    await screen.findByText("Liga Catán 2026");
    fireEvent.click(
      screen.getByRole("button", { name: "admin-add-participants" }),
    );
  ...
  it("groups tab renders the groups view", async () => {
    setupTorneo(makeTorneo({ format: "groups" }));
    server.use(
      http.get("/api/torneos/:id/groups", () =>
        HttpResponse.json({ groups: [], currentPhase: 1 }),
      ),
    );
    renderDetail();
    await screen.findByText("Liga Catán 2026");
