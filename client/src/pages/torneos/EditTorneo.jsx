...
export default function EditTorneo() {
  ...
  if (loading) return <div className={styles.page}><div className={styles.inner}><p className={styles.cardDate}>Cargando…</p></div></div>
  if (notFound) return <div className={styles.page}><div className={styles.inner}><p className={styles.cardDate}>Torneo no encontrado</p></div></div>

  return (
    <div className={styles.page}>
      <Helmet><title>Editar torneo – Turnocero</title></Helmet>
      <div className={styles.inner}>
        <div className={styles.formHeader}>
          <Link to={`/torneos/${id}`} className={styles.backLink}>← Volver al torneo</Link>
          <h1 className={styles.title}>Editar torneo</h1>
        </div>

        <form className={styles.formCard} onSubmit={handleSubmit}>
          <ImageDropzone preview={preview || currentImage} onFile={handleFile} />
          {file && <span className={styles.editImageHint}>Nueva imagen seleccionada — se reemplazará al guardar</span>}
          ...
          {['draft', 'registration'].includes(torneoStatus) && (
            <label className={styles.field}>
              ...
              <div className={styles.formatOptions}>
                ...
                >
                  <span className={styles.formatIcon}>👤</span>
                  <span className={styles.formatName}>Yo agrego participantes</span>
                </button>
              </div>
            </label>
          )}
          ...
          {torneoFormat === 'groups' && torneoStatus === 'draft' && (
            <div className={styles.groupsConfig}>
              <h3 className={styles.groupsConfigTitle}>Configuración Grupos (editable solo en borrador)</h3>
              <div className={styles.groupsConfigGrid}>
                <label className={styles.field}>
                  <span className={styles.fieldLabel}>Jugadores por mesa</span>
                  <input className={styles.fieldInput} type="number" min={2} max={12}
                    value={tableSize} onChange={(e) => setTableSize(e.target.value)} />
                </label>
                <label className={styles.field}>
                  <span className={styles.fieldLabel}>Partidas por grupo</span>
                  <input className={styles.fieldInput} type="number" min={1} max={12}
                    value={gamesPerGroup} onChange={(e) => setGamesPerGroup(e.target.value)} />
                </label>
                <label className={styles.field}>
                  <span className={styles.fieldLabel}>Clasificados por grupo</span>
                  <input className={styles.fieldInput} type="number" min={1}
                    value={qualifiersPerGroup} onChange={(e) => setQualif(e.target.value)} />
                </label>
              </div>
            </div>
          )}
          ...
          <div className={styles.formActions}>
            <Link to={`/torneos/${id}`} className={styles.btnGhost}>Cancelar</Link>
            <button type="submit" className={styles.btnPrimary} disabled={submitting}>
              {submitting ? 'Guardando…' : 'Guardar cambios'}
            </button>
          </div>
          <p className={styles.formHint}>
            Nota: el formato del torneo no se puede editar después de creado. La configuración de Grupos solo es editable en borrador.
          </p>
        </form>
      </div>
    </div>
