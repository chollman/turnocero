...
describe('<NoticiaDetail>', () => {
  it('renders the title + body once loaded', async () => {
    ...
    renderDetail();
    expect(await screen.findByRole('heading', { name: 'Título de la noticia' })).toBeInTheDocument();
    expect(screen.getByText(/cuerpo extenso/i)).toBeInTheDocument();
  });

  it('renders the external link when provided', async () => {
    setupNoticia(makeNoticia({ link: 'https://x.com/y', linkLabel: 'Leer más' }));
    renderDetail();
  ...
  it('shows admin actions (Editar/Eliminar) only for admins', async () => {
    ...
    await screen.findByRole('heading', { name: 'Título de la noticia' });
    expect(screen.queryByRole('button', { name: /eliminar/i })).toBeInTheDocument();
  });
  ...
  it('shows the not-found state on 404', async () => {
    server.use(http.get('/api/noticias/:id', () => HttpResponse.json({}, { status: 404 })));
    renderDetail();
    expect(await screen.findByText(/noticia no encontrada/i)).toBeInTheDocument();
  });
});
