...
describe("<Noticias>", () => {
  ...
  it("admin: NoticiaCard shows Editar and Eliminar buttons", async () => {
    setup({
      ...
      noticias: [makeNoticia({ title: "Noticia edit test" })],
    });
  ...
  it('admin: clicking "Editar" on a NoticiaCard opens the edit form', async () => {
    setup({
      user: { _id: "admin", isAdmin: true },
      noticias: [makeNoticia({ title: "Editable" })],
    });
  ...
  it("admin: saving an edited NoticiaCard PUTs and updates the title", async () => {
    ...
    server.use(
      http.put(`/api/noticias/${original._id}`, () =>
  ...
  it("admin: cancelling edit closes the form", async () => {
    setup({
      user: { _id: "admin", isAdmin: true },
      noticias: [makeNoticia({ title: "Sin cambios" })],
    });
  ...
  it('admin: clicking "Eliminar" shows confirmation row', async () => {
    setup({
      user: { _id: "admin", isAdmin: true },
      noticias: [makeNoticia({ title: "Para borrar" })],
    });
  ...
  it("admin: confirming delete removes the noticia from the list", async () => {
    ...
    server.use(
      http.delete(`/api/noticias/${n._id}`, () =>
    ...
    await waitFor(() =>
      expect(screen.queryByText("A eliminar")).not.toBeInTheDocument(),
    );
  });
  ...
  it('admin: clicking "No" in confirm row cancels deletion', async () => {
    setup({
      user: { _id: "admin", isAdmin: true },
      noticias: [makeNoticia({ title: "No borrar" })],
    });
    ...
    expect(
      screen.queryByRole("button", { name: /^sí$/i }),
    ).not.toBeInTheDocument();
  });
  ...
  it("shows skeletons while loading", async () => {
    ...
    expect(
      screen.queryByRole("heading", { name: "Noticias" }),
    ).toBeInTheDocument();
    resolve?.();
    await waitFor(() =>
  ...
  it('admin: "Ver más noticias" loads additional page', async () => {
    ...
    expect(
      screen.getByRole("button", { name: /ver más noticias/i }),
    ).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: /ver más noticias/i }));
    await waitFor(() =>
      expect(screen.getByText("Noticia p2")).toBeInTheDocument(),
    );
  ...
  it("NoticiaCard with image shows a button (lightbox toggle)", async () => {
    const n = makeNoticia({
      image: { url: "https://example.com/img.jpg", publicId: "abc" },
    });
    setup({ noticias: [n] });
