...
function renderCard({ table = makeTable(), userId = 'host1', isPast = false } = {}) {
  return render(
    <MemoryRouter>
...
describe('<FeedCard>', () => {
  ...
  it('renders the meta line with location and player count', () => {
    renderCard({ table: makeTable({ location: 'Córdoba', maxPlayers: 4, players: [{ _id: 'p1' }] }) });
  ...
  it('links "Ver →" to /mesas/:id', () => {
    renderCard();
    expect(screen.getByRole('link', { name: /Ver/i })).toHaveAttribute('href', '/mesas/t1');
  });
