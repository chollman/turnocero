...
describe('<Messages>', () => {
  it('renders the page heading + empty state when there are no conversations', async () => {
    ...
    expect(await screen.findByText(/no hay conversaciones|sin conversaciones|empez[aá]/i)).toBeInTheDocument();
  });
  ...
  it('renders the conversation list when ChatContext has conversations', async () => {
    setup({
      conversations: {
        f1: {
          user: { _id: 'f1', username: 'alice', avatar: { url: '', publicId: '' } },
          messages: [{ _id: 'm1', content: 'last msg', createdAt: new Date().toISOString() }],
          unread: 0,
        },
      },
    });
  ...
  it('selecting a friend on desktop calls openChat (no navigate)', async () => {
    ...
    await screen.findByText('alice');
    fireEvent.click(screen.getByText('alice'));
    expect(openChat).toHaveBeenCalledWith(expect.objectContaining({ _id: 'f1', username: 'alice' }));
