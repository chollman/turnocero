...
describe('<FingerSelector>', () => {
  it('starts in idle phase showing the prompt', () => {
    renderApp();
    expect(screen.getByText(/pongan los dedos en la pantalla/i)).toBeInTheDocument();
  ...
  it('starts countdown once two fingers are down', () => {
    ...
    const screenEl = container.firstChild;
    act(() => {
      fireTouch(screenEl, 'touchstart', [makeTouch(1, 100, 100), makeTouch(2, 200, 200)]);
    });
  ...
  it('renders one dot per active touch', () => {
    ...
    const wrappers = Array.from(dots).filter((el) => el.classList.value.match(/\bdot(_|$)/) || el.style.left);
