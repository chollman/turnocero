...
vi.mock("./DatabaseSkeleton", () => ({
  default: () => <div data-testid="db-skeleton" />,
}));
...
describe("<DatabaseViewer>", () => {
  ...
  it("renders collection tabs after fetching /admin/collections", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByRole("button", { name: "compartidas" }),
      ).toBeInTheDocument();
    });
  });
  ...
  it("shows error when /admin/collections fails", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/no se pudieron cargar las colecciones/i),
      ).toBeInTheDocument();
    });
  });
