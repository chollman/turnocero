...
describe('<PanelAdmin>', () => {
  it('redirects to /login when no user is logged in', () => {
    useAuthMock.mockReturnValueOnce({ user: null, isActuallyAdmin: false, loading: false });
  ...
  it('renders nothing while auth is loading', () => {
    useAuthMock.mockReturnValueOnce({ user: null, isActuallyAdmin: false, loading: true });
    const { container } = renderPanel();
  ...
  it('reflects the enabled state on each toggle via aria-checked', () => {
    renderPanel();
    expect(screen.getByRole('switch', { name: /desactivar mesas/i })).toHaveAttribute('aria-checked', 'true');
    expect(screen.getByRole('switch', { name: /activar noticias/i })).toHaveAttribute('aria-checked', 'false');
    expect(screen.getByRole('switch', { name: /activar mi feed/i })).toHaveAttribute('aria-checked', 'false');
  });
  ...
  it('shows error box when updateConfig rejects', async () => {
    const updateConfig = vi.fn().mockRejectedValue({ response: { data: { message: 'Forbidden' } } });
    useSiteConfigMock.mockReturnValueOnce({
      sections: { mesas: { enabled: true } },
      loaded: true,
      updatedAt: null,
