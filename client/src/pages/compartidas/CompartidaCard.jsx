...
const PRIVACY_LABELS = { public: 'Público', friends: 'Amigos', private: 'Solo yo' }
...
function Polaroid({ image, index = 0, count = 1, withTape = true, caption }) {
  ...
  return (
    <div className={`${styles.polaroid} ${rotateClass}`}>
      {withTape && <span className={`${styles.polaroidTape} ${styles[`tape_${tapePos}`]}`} aria-hidden="true" />}
      <div className={`${styles.polaroidPhoto} ${count === 1 ? styles.polaroidPhotoLandscape : ''}`}>
        {image?.url ? (
      ...
      <div className={styles.header}>
          ...
          <div className={styles.authorMeta}>
            ...
            <div className={styles.metaLine}>
              <span className={styles.metaTime}>◆ hace {timeAgo(post.createdAt)}</span>
        ...
        {imageCount > 0 && (
          <div className={`${styles.photos} ${imageGridClass} ${featured ? styles.photosFeatured : ''}`}>
        ...
        {table && (
          <div className={styles.mesaTicket}>
            <div className={styles.mesaTile}>
              <GameTile game={table.boardGame} seed={table._id?.charCodeAt(0) || 42} size={38} />
            </div>
