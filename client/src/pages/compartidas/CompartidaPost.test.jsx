...
vi.mock('./CompartidaCard', () => ({
  default: ({ post }) => <div data-testid="compartida-card">{post.body || post.title}</div>,
}));
...
describe('<CompartidaPost>', () => {
  ...
  it('shows 404 error when post does not exist', async () => {
    server.use(http.get('/api/compartidas/:id', () => HttpResponse.json({}, { status: 404 })));
    renderPage();
    expect(await screen.findByText(/no existe o fue eliminada/i)).toBeInTheDocument();
  });

  it('shows 403 error when user lacks access', async () => {
    server.use(http.get('/api/compartidas/:id', () => HttpResponse.json({}, { status: 403 })));
    renderPage();
    expect(await screen.findByText(/no ten[eé]s acceso/i)).toBeInTheDocument();
  });

  it('shows a generic error on other failures', async () => {
    server.use(http.get('/api/compartidas/:id', () => HttpResponse.json({}, { status: 500 })));
    renderPage();
