...
function Probe() {
  ...
  return (
    <div>
      ...
      <button onClick={() => n.setActiveEvento("ev1")}>
        active-evento-ev1
      </button>
      <button onClick={() => n.setAdminChatActive(true)}>
        admin-active-on
      </button>
      <button
        onClick={() =>
          n.addToast({ type: "noticia", noticiaId: "n1", title: "t" })
        }
      >
        add-toast
      </button>
      <button onClick={() => n.toasts[0] && n.dismissToast(n.toasts[0].id)}>
        dismiss-first
      </button>
...
describe("NotificationContext", () => {
  ...
  it("addToast caps total visible toasts to 4", () => {
    ...
    expect(
      Number(screen.getByTestId("toasts").textContent),
  ...
  it("setActiveTable(id) marks that table as read automatically", async () => {
    server.use(
      http.get("/api/notifications", () =>
        HttpResponse.json([
          { type: "chat", tableId: "t1", count: 3, read: false, timestamp: 1 },
        ]),
      ),
    );
    renderApp();
  ...
  it("setAdminChatActive(true) marks admin chat as read", async () => {
    server.use(
      http.get("/api/notifications", () =>
        HttpResponse.json([
          { type: "admin_chat", count: 5, read: false, timestamp: 1 },
        ]),
      ),
    );
    renderApp();
  ...
  it("table:comment adds/increments a notification + toast (count comes from server payload)", () => {
    renderApp();
    // Server sends absolute count post-upsert (contrato post-emitNotification).
    fireSocketEvent("table:comment", {
      tableId: "t1",
      tableName: "Mesa A",
      commenterUsername: "bob",
      commentPreview: "genial",
      timestamp: 1,
      notifId: "n1",
      count: 1,
    });
    expect(screen.getByTestId("count").textContent).toBe("1");
    expect(screen.getByTestId("toasts").textContent).toBe("1");
    // Second event: server already incremented → emite count: 2 absoluto.
    fireSocketEvent("table:comment", {
      tableId: "t1",
      tableName: "Mesa A",
  ...
  it("table:image adds/increments a notification + toast (count comes from server payload)", () => {
    renderApp();
    fireSocketEvent("table:image", {
      tableId: "t1",
      tableName: "Mesa A",
  ...
  // ── Regresión doble-conteo ───────────────────────────────────────────
  // Antes: cada listener hacía `count: n.count + 1` localmente, y el server
  // hacía $inc por su lado → si el mismo evento llegaba dos veces (reconexión,
  // StrictMode, race con el GET) el cliente quedaba off-by-one. Ahora el
  // cliente setea `count: payload.count` (absoluto, idempotente).
  it("regresión: el mismo evento con el mismo count llegando dos veces NO double-incrementa", () => {
    renderApp();
    fireSocketEvent("table:comment", {
      tableId: "t1",
      tableName: "A",
    ...
    expect(parseInt(screen.getByTestId("unread").textContent)).toBe(3);
    // Replay del mismo payload (count absoluto 3). El handler debe ser idempotente.
    fireSocketEvent("table:comment", {
      tableId: "t1",
      tableName: "A",
      commenterUsername: "bob",
    ...
    expect(parseInt(screen.getByTestId("unread").textContent)).toBe(3); // Sigue en 3, NO en 6
  });

  it("regresión: markRead resetea el count a 0 para que el próximo evento NO acumule sobre el viejo", async () => {
    renderApp();
    // Acumular hasta 5
    fireSocketEvent("chat:notification", {
      tableId: "t1",
    ...
    expect(parseInt(screen.getByTestId("unread").textContent)).toBe(1); // 1, no 6
  });

  it("table:spot-opened adds a notification + toast", () => {
    renderApp();
    fireSocketEvent("table:spot-opened", {
      tableId: "t5",
      tableName: "Mesa E",
      timestamp: 1,
    });
  ...
  it("table:cancelled adds a notification + toast", () => {
    renderApp();
    fireSocketEvent("table:cancelled", {
      tableId: "t6",
      tableName: "Mesa F",
      timestamp: 1,
    });
  ...
  it("torneo:registration-accepted adds a notification + toast", () => {
    renderApp();
    fireSocketEvent("torneo:registration-accepted", {
      torneoId: "tn1",
      torneoTitle: "Copa Test",
      round: 1,
    });
    expect(screen.getByTestId("count").textContent).toBe("1");
    expect(screen.getByTestId("toasts").textContent).toBe("1");
  ...
  it("torneo:registration-rejected adds a notification + toast", () => {
    renderApp();
    fireSocketEvent("torneo:registration-rejected", {
      torneoId: "tn1",
      torneoTitle: "Copa Test",
    });
    expect(screen.getByTestId("count").textContent).toBe("1");
  ...
  it("torneo:advanced adds a notification + toast", () => {
    renderApp();
    fireSocketEvent("torneo:advanced", {
      torneoId: "tn1",
      torneoTitle: "Copa Test",
      round: 2,
    });
  ...
  it("torneo:eliminated adds a notification + toast", () => {
    renderApp();
    fireSocketEvent("torneo:eliminated", {
      torneoId: "tn1",
      torneoTitle: "Copa Test",
      round: 2,
    });
  ...
  it("torneo:started and torneo:finished add notifications", () => {
    renderApp();
    fireSocketEvent("torneo:started", {
      torneoId: "tn2",
      torneoTitle: "Liga",
      round: 0,
    });
    fireSocketEvent("torneo:finished", {
      torneoId: "tn3",
      torneoTitle: "Copa",
      round: 0,
    });
  ...
  it("compartida:comment adds/increments a notification + toast (count from server payload)", () => {
    renderApp();
    fireSocketEvent("compartida:comment", {
      compartidaId: "c1",
      compartidaTitle: "Post A",
      commenterUsername: "bob",
      commentPreview: "nice",
      timestamp: 1,
      notifId: "cn1",
  ...
  it("compartida:like adds/increments a notification + toast (count from server payload)", () => {
    renderApp();
    fireSocketEvent("compartida:like", {
      compartidaId: "c1",
      compartidaTitle: "Post A",
      fromUsername: "bob",
      timestamp: 1,
