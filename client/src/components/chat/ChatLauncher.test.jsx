...
function setup({
  ...
  useSiteConfig.mockReturnValue({ isSectionEnabled: (k) => (k === 'dms' ? dmsEnabled : true) });
...
describe('<ChatLauncher>', () => {
  ...
  it('search input filters friends by username (case-insensitive)', async () => {
    setup({
      friends: [FRIEND({ username: 'alice' }), FRIEND({ username: 'bob' }), FRIEND({ username: 'carol' })],
    });
  ...
  it('clicking a friend on desktop calls openChat', async () => {
    ...
    expect(openChat).toHaveBeenCalledWith(expect.objectContaining({ username: 'alice' }));
