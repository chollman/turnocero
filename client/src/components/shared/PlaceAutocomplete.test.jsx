...
describe('<PlaceAutocomplete>', () => {
  ...
  it('debounces fetchAutocompleteSuggestions (300ms)', async () => {
    ...
    fireEvent.change(screen.getByRole('textbox'), { target: { value: 'Av Corrientes' } });
    await tick(100);
    expect(mockFetchSuggestions).not.toHaveBeenCalled();
  ...
  it('renders suggestions in a dropdown after fetch', async () => {
    mockFetchSuggestions.mockResolvedValue({
      suggestions: [
        suggestion({ placeId: 'a', main: 'Av. Corrientes 1234', secondary: 'CABA', lat: -34.6, lng: -58.4, formatted: 'Av. Corrientes 1234, CABA' }),
        suggestion({ placeId: 'b', main: 'Av. Corrientes 5678', secondary: 'CABA', lat: -34.7, lng: -58.5, formatted: 'Av. Corrientes 5678, CABA' }),
      ],
    });
    render(<PlaceAutocomplete value="" onChange={() => {}} />);
    fireEvent.change(screen.getByRole('textbox'), { target: { value: 'Av Corrientes' } });
  ...
  it('calls onSelect with coords + formattedAddress when a suggestion is picked', async () => {
    ...
    mockFetchSuggestions.mockResolvedValue({
      suggestions: [
        suggestion({ placeId: 'a', main: 'Florida 100', secondary: 'CABA', lat: -34.6, lng: -58.4, formatted: 'Florida 100, CABA' }),
      ],
    });
    render(<PlaceAutocomplete value="" onChange={onChange} onSelect={onSelect} />);
    fireEvent.change(screen.getByRole('textbox'), { target: { value: 'Florida' } });
    await tick();
  ...
  it('closes the dropdown after picking', async () => {
    mockFetchSuggestions.mockResolvedValue({
      suggestions: [
        suggestion({ placeId: 'a', main: 'X', secondary: 'Y', lat: 1, lng: 2, formatted: 'X, Y' }),
      ],
    });
    render(<PlaceAutocomplete value="" onChange={() => {}} onSelect={() => {}} />);
  ...
  it('renders a plain input when API key is missing', () => {
    ...
    const onChange = vi.fn();
    render(<PlaceAutocomplete value="hola" onChange={onChange} placeholder="Ej" />);
