  ...
  default: ({ onCropComplete, onCropChange, onZoomChange }) => {
    ...
    return (
      <div data-testid="mock-cropper">
        <button onClick={() => onCropChange?.({ x: 5, y: 5 })}>simulate-drag</button>
...
describe('<AvatarCropModal>', () => {
  it('renders nothing when open is false', () => {
    const { container } = render(
      <AvatarCropModal open={false} file={null} onCancel={() => {}} onConfirm={() => {}} />,
    );
  ...
  it('renders header + cropper + zoom slider + actions when open with a file', () => {
    render(
      <AvatarCropModal open file={makeFile()} onCancel={() => {}} onConfirm={() => {}} />,
    );
    expect(screen.getByRole('dialog', { name: 'Recortar avatar' })).toBeInTheDocument();
  ...
  it('Cancel triggers onCancel', () => {
    ...
    render(
      <AvatarCropModal open file={makeFile()} onCancel={onCancel} onConfirm={() => {}} />,
    );
  ...
  it('Save is initially disabled (no crop data yet) and becomes enabled after the cropper reports pixels', async () => {
    ...
    render(
      <AvatarCropModal open file={makeFile()} onCancel={() => {}} onConfirm={onConfirm} />,
    );
