...
async function handleBgWatch(url, bggUsername, apiUrl) {
  ...
  if (typeof data.partidas === 'number') {
    descParts.push(`${data.partidas} ${data.partidas === 1 ? 'partida registrada' : 'partidas registradas'} en BGG`)
  }
  if (typeof data.juegos === 'number') {
    descParts.push(`${data.juegos} ${data.juegos === 1 ? 'juego en colección' : 'juegos en colección'}`)
  }
  ...
  const image = hasThumb ? data.topGame.thumbnail : `${url.origin}/og-default.png`
