...
**Current coverage** (2026-05-18, twelfth session): server ~40% lines (utilities ~80%, routes 20-90%); client **67% lines / 63.85% statements** (utils 98%, shared/admin ~80-100%, torneos components ~90%+, BG Watch panels 63%+ + CreatePlayModal, admin pages 100%, utilidades 100%, layout components 80%+, **ThemeContext + SiteConfigContext + AuthContext cubiertos** (26 tests entre los 3), Sidebar logout flow, auth pages ~89%, me/messages ~88-91%). **Total 880 tests** (193 server + 687 client). Plan and rollout tracked in [plans/testing-infrastructure.md](plans/testing-infrastructure.md).
