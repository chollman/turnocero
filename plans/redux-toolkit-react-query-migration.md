...
- ✅ **eventos** (7 archivos con axios directo — Eventos, EventoDetail, EventoInscripciones, EventoLudotecaPicker migrados; EventoForm/EventoLudoteca/EventoMesas/useEventoSocket quedaron sin cambios). `queries/eventos.js`: `eventoKeys` con sub-keys anidadas bajo `detail(id)` (`inscripciones`, `ludoteca`, `mesas`), igual que `torneoKeys` en la fase anterior. `useEventosQuery` es `useInfiniteQuery` (lista principal); sus 4 handlers de socket (`created`/`updated`/`counts-changed`/`deleted`) parchean directo la forma anidada `{pages: [...]}` del cache — mapean/filtran todas las páginas para updates cross-page, o solo `pages[0]` para el prepend-on-create con dedupe por `_id`. Insight nuevo (`EventoDetail.jsx`): la ludoteca no tiene su propio GET — es un array embebido en el evento — así que `useEventoLudotecaQuery` es un query cache-only (`enabled:false`) sembrado una vez por `id` desde `evento.ludoteca` vía `useEffect` con dep `!!evento` (booleano, no la referencia) para no re-sembrar en cada patch posterior; el mismo patrón se repitió para `mesas`. Los props `items`/`setItems` que `EventoDetail` ya le pasaba a `EventoLudoteca.jsx`/`EventoMesas.jsx` no cambiaron de forma — sólo el `setItems` del padre pasó de `useState` a un shim que escribe al cache — así que esos dos hijos quedaron intactos. `onDeleted` del socket necesitaba forzar el estado "not found" sin esperar un refetch/404: se resolvió con un booleano local `forcedGone` OR'd al `isError` derivado de la query (no hay forma de escribir el estado de error de una query a mano). `queries/bgg.js` (compartido con mesas) ganó `useBggCollectionQuery` para el tab "Mi colección" de `EventoLudotecaPicker`. Sin Redux — cero estado cross-cutting genuino, igual que mesas/torneos. Verificado en browser: crear evento (prepend al cache de la lista), detalle, agregar juego a la ludoteca vía búsqueda BGG (POST 201 + shim de cache), tab Mesas vacío, panel de inscripciones (carga limpia, counts en 0), ambos temas, consola y logs de server limpios (único ruido: `REQUEST_DENIED` de Google Geocoding por restricción de IP en el entorno dev, no relacionado a esta migración). Cerrado 2026-08-02.
- ✅ **compartidas** (9 de 12 archivos con axios directo — Compartidas, CompartidaPost, CompartidaCard, ResenaCard, CompartidaComments, CompartidasSidebar, CreateCompartidaForm, ReviewsByGame, BgWatchHomeWidget; `createJuntada.js` quedó sin cambios — orquestador de 2 fases compartido con BG Watch, ya extraído). `queries/compartidas.js`: `compartidaKeys` con sub-key `comments(id)`; `useCompartidasFeedQuery`/`useReviewsByGameQuery`/`useCompartidaCommentsQuery` son las tres `useInfiniteQuery` del dominio. El feed principal (`Compartidas.jsx`) parchea `{pages:[...]}` para create/update/delete, sincronizando también el campo `featured` de `pages[0]` cuando corresponde (mismo criterio que eventos). Insight nuevo (`CompartidaComments.jsx`, la migración más intrincada de la fase): el componente necesitaba un contador `total` propio que sobrevive a los patches locales del árbol de comentarios (add/reply/delete) SIN resetearse en cada re-render de la query — se resolvió resincronizando `total` solo cuando cambia `pages.length` (nueva página cargada), no en cada `data` nuevo (que también cambia tras cada `setQueryData` local). `CompartidaCard.jsx`/`ResenaCard.jsx` no cambiaron su modelo de estado local (`post` vía `useState(initialPost)` + bubbling `onUpdated`/`onDeleted`) — solo las llamadas axios pasaron a las funciones planas de `queries/compartidas.js`; el padre (`Compartidas.jsx`) es quien traduce esos callbacks a patches de cache. `CompartidasSidebar.jsx` consume dos queries nuevas en `queries/tables.js` (`useTopGamesQuery`, `useMyUpcomingTablesQuery`) — pertenecen ahí por API, no por ubicación de archivo (mismo criterio que `useUserSearchQuery` en torneos.js). `BgWatchHomeWidget.jsx` (usa `/api/bgg/partidas`) sumó `useBgWatchHomeStatsQuery` a `queries/bgg.js` por la misma razón; el fallback i18n de "juego desconocido" se movió del cálculo (`queryFn` no tiene acceso a `t()`) al render. `CreateCompartidaForm.jsx` reemplazó sus dos fetches de mesa/evento prefilled por los `useTableQuery`/`useEventoQuery` ya existentes de fases anteriores, ganando cache compartida gratis. Sin Redux. Verificado en browser: crear/editar/borrar juntada, like, comentar/editar/likear/borrar comentario (conteo se resincroniza en cada paso), tabs Reseñas/Juntadas, ReviewsByGame, sidebar (tus mesas + top juegos), ambos temas, consola y logs de server limpios. Cerrado 2026-08-03.
- ✅ **bg-watch** (25 archivos con axios directo — el dominio más grande de la fase: VariantPicker, ExpansionsPicker, ComunidadJuegoDetail, BgWatchH2H, ComunidadRankBadge, GroupStatsPanel, PlayDetail, MyGamesPicker, LocationPicker, PlayerPicker, PlayerEditModals, LocationEditModals, JugadoresPanel, UbicacionesPanel, JugadorDetail, UbicacionDetail, PartidasPanel, ColeccionPanel, PlayForm, CreatePlay, EditPlay, BgWatchComunidad, BgWatchProfile, BgWatchPerGameView; `useBggUserMap.js` refactorizado como wrapper del hook nuevo). `queries/bgWatch.js` (nuevo, el archivo de queries más grande del proyecto): agrupa todas las lecturas 1-shot/paginadas por página-número (jugadores/ubicaciones, con `invalidateQueries` de prefijo corto reemplazando el viejo patrón `refreshKey`-remount) + 4 `useInfiniteQuery` (`misJuegos`, `misUbicaciones`, `misJugadores`, `turnoceroJugadores`, `comunidadActividad`) + mutaciones planas de curación (rename/merge/avatar de jugadores y ubicaciones) + CRUD de partidas (`createBgWatchPlay`/`updateBgWatchPlay`/`deleteBgWatchPlay`). `queries/bgg.js` ganó `useBggGameQuery` (compartido genuinamente entre la vista por-juego, `CreatePlay` y `PlayForm` para la duración sugerida). `queries/tables.js` ganó `useTopGamesQuery`/`useMyUpcomingTablesQuery` (siguiendo el mismo criterio "la key vive en su dominio de API, no en la carpeta del caller" ya aplicado en fases anteriores). Patrón nuevo de esta fase — **cooldown cache-only companion**: `PartidasPanel`/`ColeccionPanel` necesitaban leer el header `x-refresh-cooldown-ms` en CADA GET (no solo tras un click manual de "Actualizar"), pero el `queryFn` solo puede devolver el payload que consumen los demás callers de la misma key. Se resolvió con una key-hermana cache-only (`enabled:false`, sembrada como side-effect dentro del `queryFn` principal vía `queryClient.setQueryData`) que el componente sincroniza a un `useState` local con un `useEffect` — permite que el countdown refleje el header tanto en la carga inicial como en el refresh manual, sin mezclar metadata de transporte con la forma de dato que otros consumidores (`EventoLudotecaPicker` para la colección) esperan. `PlayerPicker.jsx` centralizó en `useBggUsernamesMapQuery` la lógica de enriquecimiento de avatares que antes vivía duplicada como un fetch manual chunked. Sin Redux — cero estado cross-cutting genuino, igual que el resto de la fase. Verificado en browser (perfil público de un usuario con datos reales — 1304 partidas, 183 juegos, 162 jugadores, 16 ubicaciones): tabs Partidas/Colección/Jugadores/Ubicaciones, refresh + cooldown independiente en Partidas y Colección, detalle de partida + panel de estadísticas de grupo (expandible), editar jugador (rename, con invalidación confirmada), mano a mano de comunidad, vista por-juego (stats server-agregadas + ranking de comunidad), hub de comunidad (4 tabs: Juegos con toggle Más jugados/En llamas, Jugadores con 4 métricas, Actividad con scroll infinito + heatmap), ambos temas, consola y logs de server limpios. El flujo de creación/edición de partida propio (`CreatePlay`/`EditPlay`/`PlayForm`, gateado a ser dueño de una cuenta BGG conectada) se verificó vía la suite completa de tests (100 tests) en vez de manualmente — la cuenta de admin de prueba disponible en el entorno no tiene BGG conectado y las escrituras van al endpoint no-oficial de BGG (`geekplay.php`, credenciales reales requeridas). Cerrado 2026-08-03.
- ✅ **usuarios** (5 archivos núcleo en `pages/users/` — UsersList, UserProfilePublic, UserProfile, MiBgWatchCard, BgWatchUserCard — + 6 archivos externos que pegan a los mismos endpoints: `queries/torneos.js` (movió `useUserSearchQuery`) → `AddParticipantModal.jsx`, `ChatLauncher.jsx`, `Messages.jsx`, `DirectChat.jsx`, `MeFeed.jsx`). `queries/users.js` (nuevo): `useUsersListQuery`/`useUserSearchQuery`/`useUserDetailQuery` (lecturas) + mutaciones planas: ban/delete (admin), 5 acciones de amistad (`sendFriendRequest`/`cancelFriendRequest`/`acceptFriendRequest`/`rejectFriendRequest`/`unfriend`), y — únicos callers en `UserProfile.jsx` — `geocodeAddress`/`connectBgg`/`syncBgg`/`disconnectBgg`/`uploadAvatar`/`removeAvatar` (endpoints de `/api/auth`, `/api/bgg`, `/api/geocode`, no `/api/users`, pero viven acá por cohesión con la página que los usa, mismo criterio que el geocode de mesas en fases anteriores). `queries/bgWatch.js` ganó `useBgWatchQuickStatsQuery` (con opción `includeExtra`) — unifica un fetch duplicado (`Promise.all` de partidas+colección con manejo de error idéntico) que vivía separado en `MiBgWatchCard.jsx` y `BgWatchUserCard.jsx` con distinta forma de estado. `useUserSearchQuery` se mudó de `queries/torneos.js` (donde vivía "temporalmente" desde la fase torneos, con comentario explícito señalando la mudanza futura) a `queries/users.js`; único caller (`AddParticipantModal.jsx`) solo cambió el import. `UsersList.jsx`: las mutaciones ban/delete ahora usan `queryClient.setQueryData(userKeys.list(params), ...)` con el MISMO objeto `params` que arma la query de lectura del propio render — evita depender de `invalidateQueries` para una lista sin paginación server-side. `ChatLauncher.jsx` reemplazó su `fetchFriends` imperativo (llamado en mount + en cada evento de socket de amistad) por `useUsersListQuery({friendsOnly:"true"})` + `queryClient.invalidateQueries` dentro del listener de socket — mismo patrón. `DirectChat.jsx` (necesitaba side-effects de éxito/error del fetch — `openChat()` y `navigate()` respectivamente, no solo mostrar datos) resolvió la migración con dos `useEffect` separados observando `data`/`isError` de `useUserDetailQuery`, en vez de forzar la lógica dentro del `queryFn`. Gotcha de testing (recurrente, mismo patrón que bg-watch): un test de `BgWatchUserCard` esperaba por un label ESTÁTICO (`"Última partida"`, presente desde el primer render) antes de verificar los valores async — como el label no cambia con la carga, `waitFor` resolvía en el primer tick sin esperar al fetch; se corrigió metiendo la aserción de datos DENTRO del mismo `waitFor`. Sin Redux — cero estado cross-cutting genuino, igual que el resto de la fase. Verificado en browser (sesión admin real): `UsersList` embebido en `/comunidades/turnocero` — búsqueda con debounce, ban+unban de un usuario real con cache actualizándose sin refetch, `UserProfilePublic` — enviar y cancelar solicitud de amistad, `BgWatchUserCard` resolviendo con datos reales (154 partidas, top-juego con thumbnail), `/perfil` — geocode manual disparado end-to-end (error esperado `REQUEST_DENIED` de Google por restricción de IP del entorno dev, mismo problema ya documentado en fases anteriores, no relacionado a esta migración), `/mensajes` con fetch real de amigos, consola y logs de server limpios. Cerrado 2026-08-03.
- ✅ **mathtrade / comunidades-admin** (últimos dos dominios de la fase, sin orden fijo entre sí — cerrados juntos). **mathtrade** (7 archivos con axios directo: EditMathTrade, MathTradeForm, ItemForm, AdminPanel, MathTrades, MathTradeDetail): `queries/mathtrade.js` (nuevo) con `mathtradeKeys` anidadas bajo `detail(id)` (`items`, `myItems`, `results`), mismo patrón que `torneoKeys`/`eventoKeys`. `useMathTradesQuery` es `useInfiniteQuery` (portada, reemplazó el acumulador manual `page`/`totalPages`/`replace` de `MathTrades.jsx` con `hasNextPage`/`fetchNextPage`). `MathTradeDetail.jsx` (la migración más intrincada del dominio) combina 4 queries independientes (`trade`, `items`, `myItems` con `enabled:!!user`, `results` con `enabled` atado a `["results","finished"].includes(trade.status)`) — insight nuevo: el salto automático a la tab "resultados" (al cargar ya publicado, o al transicionar status vía `AdminPanel`) NO debía dispararse en cada re-render/refetch, solo en la transición real a publicado — se resolvió con un `useRef` guardando el status anterior y comparando `wasPublished` vs `isPublished` en un `useEffect([trade?.status])`, en vez del `setTab` incondicional que hacía el código viejo dentro del then de cada fetch. `onTradeUpdated` (callback que `AdminPanel` dispara tras cambiar el status) ahora hace `queryClient.setQueryData(mathtradeKeys.detail(id), data)` en vez de un segundo GET, y limpia explícitamente `mathtradeKeys.results(id)` a `null` cuando el nuevo status deja de estar publicado (el auto-disable de `enabled` no vacía el cache viejo por sí solo, a diferencia de lo que hacía el `setResults(null)` original). **comunidades-admin** (`CommunitiesAdmin.jsx`, 7 llamadas axios — GET lista, POST crear, DELETE, POST reasignar-a-base, PUT editar, PUT skin, POST logo): las lecturas/mutaciones de ADMIN se agregaron a `queries/community.js` (el archivo ya existente del dominio comunidades de la Fase 5, que solo cubría `MIAS`/tenant de usuario normal) bajo una sección separada con su propia key `communityKeys.adminList()` — mismo archivo, dos audiencias distintas (preferencias de usuario vs gestión global), documentado con un comentario que marca el corte. Todas las mutaciones de escritura invalidan `adminList()` en vez de mantener el `await load()` manual que hacía refetch completo — comportamiento idéntico, menos código. Sin Redux en ninguno de los dos dominios. Con esto se cierra por completo la Fase 6 (todos los dominios de `pages/` migrados: mesas, torneos, eventos, compartidas, bg-watch, usuarios, mathtrade, comunidades-admin). Verificado en browser: portada Math Trade con "Ver más" (scroll infinito por status), crear+eliminar una oferta en un trade abierto (conteo de participantes actualizado sin refetch manual), trade con resultados publicados (salto automático a la tab correcta), `EditMathTrade`, panel Comunidades del admin (toggle de sección con PUT + refetch confirmado por network), ambos temas, consola y logs de server limpios. Cerrado 2026-08-03.
...
**Cerrado 2026-08-03.** `authSlice.js` (nuevo) guarda `{ token, viewAsUser }`. A diferencia de theme/language (que bakean su valor inicial en `initialState` porque con correr una vez por import alcanza), acá el valor inicial depende de `localStorage` por caller — cada test necesita un store fresco reflejando su propio `localStorage`. Se resolvió exportando `getInitialAuthState()` como getter invocable aparte (no bakeado en `createSlice()`), pasado vía `configureStore({ preloadedState: { auth: getInitialAuthState() } })` en cada call site (`store.js` real + un `makeStore()` nuevo en el test). La función también sincroniza el header de Axios de entrada, mismo trabajo que hacía el efecto de boot viejo. `queries/auth.js` (nuevo): `useMeQuery({enabled})` + 9 funciones planas (login/register/oauthLogin/verifyEmail/resend/forgot/reset/logout/updateProfile requests) — sin `useMutation`, mismo criterio que el resto de la Fase 6. `AuthContext.jsx` se reescribió por dentro pero **mantiene el mismo Provider/Context** — la superficie pública de `useAuth()` (15 campos, 65 archivos consumidores) quedó byte-a-byte idéntica; internamente ahora es `useAppSelector`/`useAppDispatch`/`useMeQuery`/`queryClient.setQueryData` en vez de `useState`. Bug real encontrado y arreglado durante el testing (no solo timing): el efecto que limpia un token inválido al bootear (`if (meIsError && token) dispatch(clearToken())`, calco del `.catch()` del boot original) se re-disparaba en CUALQUIER 401 posterior de `/me` — incluido el que dispara la propia query al pasar de `enabled:false` a `enabled:true` justo después de un `login()`/`oauthLogin()`/`verifyEmail()` recién exitoso (si el mock/backend de `/me` no conocía todavía el token nuevo), pisando el token que se acababa de guardar. El original solo corría esa limpieza UNA VEZ dentro del efecto de boot (`useEffect(() => {...}, [])`); con `useMeQuery` viva toda la sesión había que replicar ese "una sola vez" a mano — se resolvió con un `useRef(token)` (`bootTokenRef`) capturado en el mount, consumido y anulado la primera vez que se usa, para que un error posterior de la query (fuera del boot) nunca dispare la limpieza. Suite completa verde: **299/299 archivos, 2969/2969 tests**, lint limpio. Verificado en browser con sesión real: login normal (200, token seteado, dashboard), logout (200, token+viewAsUser limpios, redirect a `/login`), view-as-user (persiste tras reload completo vía `preloadedState`, banner correcto, toggle ida y vuelta), registro (201, redirect a `/verificar-email` sin token), forgot-password (confirmación mostrada), botón OAuth Google renderiza sin errores de consola, SSO de subdominio (`?tenant=<slug>` sin token local dispara igual `GET /api/auth/me`, sin crashear ni loopear). **Limitación del entorno, no del código:** verificar-email y reset-password terminan en un código/token que el server manda por email real (Resend activo) y guarda hasheado en Mongo — no hay forma de completá esos dos pasos sin acceso a la bandeja de entrada real; el riesgo residual es bajo porque `verifyEmail()` comparte exactamente el mismo patrón (`dispatch(setToken)` + `setQueryData`) que `login()`/`oauthLogin()`, ambos sí verificados end-to-end, y el bug real encontrado (arriba) afectaba a los tres por igual y ya está arreglado. OAuth Facebook no tiene botón en este entorno de dev (`VITE_FB_APP_ID` no configurado) — preexistente, no evaluable acá.

**Pregunta abierta para Fase 8:** el texto de Fase 8 dice "confirmar que ningún archivo importa `context/AuthContext.jsx`... y borrarlo", calcado de cómo se cerró ThemeContext/LanguageContext (Fase 1: reemplazadas por hooks Redux directos, Context eliminado). Acá se decidió lo contrario a propósito — mantener el Provider/Context tal cual por el radio de impacto (65 archivos con `useAuth()`) — así que ese punto de Fase 8 no aplica a AuthContext sin más discusión: o se lo tacha de la lista, o se hace aparte un refactor de los 65 call sites a un hook Redux/Query directo. Definir con el usuario antes de tocarlo.
