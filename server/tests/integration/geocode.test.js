...
describe('GET /api/geocode', () => {
  it('requires authentication', async () => {
    const res = await request(app).get('/api/geocode').query({ q: 'Av Corrientes 1234' });
    expect(res.status).toBe(401);
  });

  it('rejects empty or too-short queries with 400', async () => {
    const { token } = await createAuthedUser();
    const res1 = await request(app).get('/api/geocode').query({ q: '' }).set(authHeader(token));
    expect(res1.status).toBe(400);
    const res2 = await request(app).get('/api/geocode').query({ q: 'ab' }).set(authHeader(token));
  ...
  it('calls Google on cache miss and saves the result', async () => {
    ...
    geocodeMock.mockResolvedValueOnce(
      googleHit({ lat: -34.6037, lng: -58.3816, formatted: 'Av. Corrientes 1234, CABA' })
    );
    ...
    // Persistió en cache.
    const cached = await GeocodeCache.findOne({ query: 'av corrientes 1234' }).lean();
  ...
  it('returns 404 when Google returns no results', async () => {
    ...
    const cached = await GeocodeCache.findOne({ query: 'asdfqwerzxcv1234' }).lean();
