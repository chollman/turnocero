...
describe('POST /api/friends/:id/request', () => {
  it('sends a request: target gets a pending request entry', async () => {
    ...
    expect(updated.friendRequests.find((r) => r.from.equals(me._id))).toBeDefined();
  });
  ...
  it('400s when a duplicate request is sent', async () => {
    ...
    await request(app).post(`/api/friends/${target._id}/request`).set('Authorization', `Bearer ${token}`);
    const res = await request(app)
      .post(`/api/friends/${target._id}/request`)
      .set('Authorization', `Bearer ${token}`);
    expect(res.status).toBe(400);
...
describe('POST /api/friends/:id/accept', () => {
  it('makes both users friends and clears the request', async () => {
    ...
    expect(meAfter.friends.map((f) => f.toString())).toContain(sender._id.toString());
    expect(senderAfter.friends.map((f) => f.toString())).toContain(me._id.toString());
    expect(meAfter.friendRequests.find((r) => r.from.equals(sender._id))).toBeUndefined();
  });
...
describe('DELETE /api/friends/:id (unfriend)', () => {
  it('removes the bidirectional friendship', async () => {
    ...
    expect(meAfter.friends.map((f) => f.toString())).not.toContain(other._id.toString());
    expect(otherAfter.friends.map((f) => f.toString())).not.toContain(me._id.toString());
  });
});
