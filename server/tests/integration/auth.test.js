...
describe('POST /api/auth/verify-email', () => {
  ...
  it('returns 400 generic on wrong code and increments attempts', async () => {
    ...
    const u = await User.findOne({ email: 'tobev@test.local' }).select('+emailVerificationAttempts');
  ...
  it('400s when fields are missing', async () => {
    const res = await request(app).post('/api/auth/verify-email').send({ email: 'a@b.c' });
    expect(res.status).toBe(400);
  });
});
...
describe('POST /api/auth/login', () => {
  ...
  it('returns 403 with code "banned" when account is banned', async () => {
    await createUser({ email: 'ban@test.local', isBanned: true, bannedReason: 'spam' });
    const res = await request(app).post('/api/auth/login').send({
...
describe('PUT /api/auth/profile', () => {
  it('updates displayName / nombre / apellido', async () => {
    ...
    const res = await request(app)
      ...
      .send({ displayName: 'Claudio H', nombre: 'Claudio', apellido: 'Hollman' });
    expect(res.status).toBe(200);
  ...
  it('requires authentication', async () => {
    const res = await request(app).put('/api/auth/profile').send({ nombre: 'x' });
    expect(res.status).toBe(401);
  });
});
...
describe('PUT /api/auth/avatar + DELETE /api/auth/avatar', () => {
  it('uploads an avatar, stores { url, publicId }, deletes prior on replace', async () => {
    ...
    const res1 = await request(app)
      .put('/api/auth/avatar')
      .set('Authorization', `Bearer ${tokenFor(user)}`)
      .attach('avatar', Buffer.from('fake-jpeg'), { filename: 'a.jpg', contentType: 'image/jpeg' });
  ...
  it('DELETE clears the avatar and calls Cloudinary.destroy', async () => {
    const user = await createUser();
    user.avatar = { url: 'https://x/y.webp', publicId: 'turnocero/users/abc/avatar' };
    await user.save();
    ...
    expect(cloudMock.cloudinary.uploader.destroy).toHaveBeenCalledWith('turnocero/users/abc/avatar');
  });

  it('requires authentication', async () => {
    const res = await request(app)
      .put('/api/auth/avatar')
      .attach('avatar', Buffer.from('x'), { filename: 'x.jpg', contentType: 'image/jpeg' });
    expect(res.status).toBe(401);
  });
});
...
describe('POST /api/auth/forgot-password', () => {
  ...
  it('emails a reset link when the account exists', async () => {
    ...
    const res = await request(app).post('/api/auth/forgot-password').send({ email: 'fp@test.local' });
    expect(res.status).toBe(200);
    expect(emailMock.__sentEmails.length).toBe(1);
