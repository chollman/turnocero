...
describe("normalizeLocationInput", () => {
  ...
  it("passes through object input with valid coords", () => {
    expect(
      normalizeLocationInput({ texto: "X", lat: -34.6, lng: -58.4 }),
    ).toEqual({
      texto: "X",
      lat: -34.6,
      lng: -58.4,
...
describe("locationForCreate", () => {
  ...
  it("falls back to user.direccion when input is empty", () => {
    ...
    expect(
      locationForCreate({ texto: "", lat: null, lng: null }, user),
    ).toEqual({
      texto: "Mi casa",
      lat: -34.6,
      lng: -58.4,
  ...
  it("returns the (empty) normalized input when both input and user are empty", () => {
    ...
    expect(locationForCreate({}, null)).toEqual({
      texto: "",
      lat: null,
      lng: null,
      displayName: "",
    });
    expect(locationForCreate({}, { texto: "", lat: null, lng: null })).toEqual({
      texto: "",
      lat: null,
      lng: null,
      displayName: "",
