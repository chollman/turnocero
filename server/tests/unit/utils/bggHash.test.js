...
function basePlay() {
  return {
    ...
    nowinstats: false,
    players: [
      { name: 'Ana',  username: 'ana',  userid: 1, position: '1', color: 'red',  score: '42', win: true,  new: false, rating: 8 },
      { name: 'Beto', username: 'beto', userid: 2, position: '2', color: 'blue', score: '30', win: false, new: false, rating: 7 },
    ],
  };
}
...
describe('utils/bggHash — computePlayHash', () => {
  ...
  it('changes when a player is added', () => {
    ...
    modified.players.push({ name: 'Cris', username: 'cris', userid: 3, position: '3', color: 'green', score: '20', win: false, new: true, rating: null });
  ...
  it('ignores extra keys on the play object', () => {
    const a = basePlay();
    const b = { ...basePlay(), gameId: '13', gameName: 'Catan', gameThumbnail: 'http://x', bggUsername: 'ana', createdAt: new Date() };
  ...
  it('treats undefined and null the same for missing fields', () => {
    const withUndef = { ...basePlay(), location: undefined, comments: undefined };
  ...
  it('handles comments with special characters', () => {
    const a = computePlayHash({ ...basePlay(), comments: 'línea 1\nlínea 2 — ñandú 🎲 "quoted" \\backslash' });
    const b = computePlayHash({ ...basePlay(), comments: 'línea 1\nlínea 2 — ñandú 🎲 "quoted" \\backslash' });
    const c = computePlayHash({ ...basePlay(), comments: 'línea 1\nlínea 2 — ñandú 🎲 "quoted" \\backslash ' });
    expect(a).toBe(b);
    expect(a).not.toBe(c);
  });
