...
async function buildAndInsertMatches(torneo) {
  ...
  const raw = torneo.format === 'league'
    ? generateLeagueFixture(ids)
    : generateSingleElimBracket(ids);

  if (raw.length === 0) {
    throw new Error('No se pudieron generar los partidos: faltan participantes.');
  }
  ...
  if (torneo.format === 'single_elim') {
    const byKey = new Map(inserted.map((doc) => [`${doc.round}-${doc.matchIndex}`, doc]));
    const ops = [];
    raw.forEach((m, idx) => {
...
async function buildAndInsertGroupsForPhase(torneo, phaseNumber, playerIds, tableSizeOverride) {
...
router.get('/', protect, requireAdmin, async (req, res) => {
  try {
    ...
    res.json({ torneos: summarized, total, page, pages: Math.ceil(total / limit) });
  } catch (err) {
...
router.post('/', protect, requireAdmin, multer.single('image'), async (req, res) => {
  try {
    const {
      title, description, game, format, maxParticipants,
      inscriptionMode, tableSize, gamesPerGroup, qualifiersPerGroup,
    } = req.body;
    if (!title?.trim() || !game?.trim() || !format) {
      return res.status(400).json({ message: 'Faltan campos obligatorios (título, juego, formato)' });
    }
    if (!['league', 'single_elim', 'groups'].includes(format)) {
      return res.status(400).json({ message: 'Formato inválido' });
    }
    if (inscriptionMode && !['open', 'admin_only'].includes(inscriptionMode)) {
      return res.status(400).json({ message: 'Modo de inscripción inválido' });
    }
    ...
    const data = {
      ...
      maxParticipants: maxParticipants ? Math.max(2, parseInt(maxParticipants)) : null,
      image,
      createdBy:       req.user._id,
    };
    if (format === 'groups') {
      data.tableSize          = tableSize          ? clamp(parseInt(tableSize), 2, 12)         : 4;
      data.gamesPerGroup      = gamesPerGroup      ? clamp(parseInt(gamesPerGroup), 1, 12)     : 3;
      data.qualifiersPerGroup = qualifiersPerGroup ? Math.max(1, parseInt(qualifiersPerGroup)) : 2;
    }

    const torneo = await Torneo.create(data);
    const populated = await Torneo.findById(torneo._id).populate('createdBy', USER_FIELDS);
...
router.get('/:id', protect, requireAdmin, async (req, res) => {
  try {
    if (!isValidId(req.params.id)) return res.status(404).json({ message: 'Torneo no encontrado' });
...
router.put('/:id', protect, requireAdmin, multer.single('image'), async (req, res) => {
  try {
    ...
    const {
      title, description, game, maxParticipants, inscriptionMode,
      tableSize, gamesPerGroup, qualifiersPerGroup,
    } = req.body;
    if (title?.trim()) torneo.title = title.trim();
    if (typeof description === 'string') torneo.description = description.trim();
    ...
    if (torneo.format === 'groups' && torneo.status === 'draft') {
      if (tableSize          !== undefined) torneo.tableSize          = clamp(parseInt(tableSize), 2, 12);
      if (gamesPerGroup      !== undefined) torneo.gamesPerGroup      = clamp(parseInt(gamesPerGroup), 1, 12);
      if (qualifiersPerGroup !== undefined) torneo.qualifiersPerGroup = Math.max(1, parseInt(qualifiersPerGroup));
    }
  ...
  try {
    ...
    if (matchCount > 0 && torneo.status !== 'draft') {
      return res.status(400).json({
        message: 'No se puede eliminar: el torneo tiene partidos generados. Cambialo a borrador primero.',
      });
    }
...
router.post('/:id/register', protect, async (req, res) => {
  try {
    ...
    if (torneo.status !== 'registration') {
      return res.status(400).json({ message: 'La inscripción no está abierta' });
    }
    const userId = req.user._id;
    if (torneo.participants.some((p) => String(p) === String(userId))) {
      return res.status(400).json({ message: 'Ya estás inscripto' });
    }
    if (torneo.pendingRegistrations.some((r) => String(r.user) === String(userId))) {
      return res.status(400).json({ message: 'Ya tenés una inscripción pendiente' });
    }
    if (torneo.maxParticipants && torneo.participants.length >= torneo.maxParticipants) {
      return res.status(400).json({ message: 'El torneo ya alcanzó el cupo máximo' });
    }
    ...
    await torneo.save();
    res.json({ message: 'Inscripción enviada, esperá la aprobación del admin' });
  } catch (err) {
...
router.delete('/:id/register', protect, async (req, res) => {
  try {
    ...
    if (torneo.pendingRegistrations.length === before) {
      return res.status(400).json({ message: 'No tenés una inscripción pendiente' });
    }
    await torneo.save();
...
router.post('/:id/participants/:userId', protect, requireAdmin, async (req, res) => {
  try {
    const torneo = await Torneo.findById(req.params.id);
    if (!torneo) return res.status(404).json({ message: 'Torneo no encontrado' });
    if (torneo.status === 'finished') {
      return res.status(400).json({ message: 'El torneo ya está finalizado' });
    }
    if (torneo.status === 'in_progress') {
      return res.status(400).json({ message: 'No se pueden agregar participantes con el torneo en curso' });
    }

    const { userId } = req.params;
    if (!isValidId(userId)) return res.status(400).json({ message: 'Usuario inválido' });

    const user = await User.findById(userId).select('_id username displayName isBanned');
    if (!user) return res.status(404).json({ message: 'Usuario no encontrado' });
    if (user.isBanned) return res.status(400).json({ message: 'No se puede agregar un usuario suspendido' });

    if (torneo.participants.some((p) => String(p) === String(userId))) {
      return res.status(400).json({ message: 'El usuario ya es participante' });
    }
    if (torneo.maxParticipants && torneo.participants.length >= torneo.maxParticipants) {
      return res.status(400).json({ message: 'El torneo alcanzó el cupo máximo' });
    }
...
router.post('/:id/registrations/:userId/accept', protect, requireAdmin, async (req, res) => {
  try {
    const torneo = await Torneo.findById(req.params.id);
    if (!torneo) return res.status(404).json({ message: 'Torneo no encontrado' });
    if (torneo.status !== 'registration') {
    ...
    const { userId } = req.params;
    const idx = torneo.pendingRegistrations.findIndex((r) => String(r.user) === String(userId));
    if (idx < 0) return res.status(404).json({ message: 'No hay inscripción pendiente para ese usuario' });
    ...
    // Notify + socket emit.
    await saveNotification(userId, 'tournament_accepted', {
      torneoId: String(torneo._id),
      torneoTitle: torneo.title,
    });
    req.app.get('io')?.to(`user:${userId}`).emit('torneo:registration-accepted', {
      torneoId: String(torneo._id),
      torneoTitle: torneo.title,
...
router.post('/:id/registrations/:userId/reject', protect, requireAdmin, async (req, res) => {
  try {
    const torneo = await Torneo.findById(req.params.id);
    if (!torneo) return res.status(404).json({ message: 'Torneo no encontrado' });
    const { userId } = req.params;
    ...
    torneo.pendingRegistrations.splice(idx, 1);
    if (!torneo.rejectedRegistrations.some((id) => String(id) === String(userId))) {
      torneo.rejectedRegistrations.push(userId);
    }
    await torneo.save();

    await saveNotification(userId, 'tournament_rejected', {
      torneoId: String(torneo._id),
      torneoTitle: torneo.title,
    });
    req.app.get('io')?.to(`user:${userId}`).emit('torneo:registration-rejected', {
      torneoId: String(torneo._id),
      torneoTitle: torneo.title,
      timestamp: new Date().toISOString(),
    });
...
router.delete('/:id/participants/:userId', protect, requireAdmin, async (req, res) => {
  try {
    const torneo = await Torneo.findById(req.params.id);
    if (!torneo) return res.status(404).json({ message: 'Torneo no encontrado' });
    if (!['draft', 'registration'].includes(torneo.status)) {
      return res.status(400).json({ message: 'No se pueden remover participantes una vez iniciado el torneo' });
    }
    const before = torneo.participants.length;
    torneo.participants = torneo.participants.filter((p) => String(p) !== String(req.params.userId));
    if (torneo.participants.length === before) {
      return res.status(404).json({ message: 'Ese usuario no es participante' });
    }
    await torneo.save();
    const populated = await Torneo.findById(torneo._id).populate('participants', USER_FIELDS);
    res.json(populated);
  } catch (err) {
...
router.patch('/:id/seeds', protect, requireAdmin, async (req, res) => {
  try {
    ...
    if (!torneo) return res.status(404).json({ message: 'Torneo no encontrado' });
    if (!['draft', 'registration'].includes(torneo.status)) {
      return res.status(400).json({ message: 'No se pueden reordenar seeds una vez iniciado el torneo' });
    }
    const { participantIds } = req.body;
    if (!Array.isArray(participantIds)) {
      return res.status(400).json({ message: 'participantIds debe ser un array' });
    }
    ...
    if (current.size !== incoming.size || [...current].some((id) => !incoming.has(id))) {
      return res.status(400).json({ message: 'El orden enviado debe contener exactamente los mismos participantes' });
    }
...
router.patch('/:id/status', protect, requireAdmin, async (req, res) => {
  try {
    ...
    if (!allowed.has(next)) {
      return res.status(400).json({ message: `Transición no permitida: ${torneo.status} → ${next}` });
    }
    // Constraint: draft → in_progress only allowed for admin_only torneos.
    if (torneo.status === 'draft' && next === 'in_progress' && torneo.inscriptionMode !== 'admin_only') {
      return res.status(400).json({
        message: 'Para arrancar el torneo abrí inscripciones primero, o configurá el modo "Yo agrego participantes".',
      });
    }
    ...
    if (next === 'finished') {
      if (torneo.format === 'groups') {
        ...
        const finalGroups = await TorneoGroup.find({ torneo: torneo._id, phase: torneo.currentPhase });
        if (finalGroups.length !== 1) {
          return res.status(400).json({
            message: 'Para finalizar un torneo de Grupos, la fase actual debe tener una sola mesa final.',
          });
        }
        ...
        if (torneo.format === 'single_elim') {
          ...
          torneo.runnerUp = final && final.winner
            ? (String(final.playerA) === String(final.winner) ? final.playerB : final.playerA)
            : null;
        } else {
    ...
    res.status(500).json({ message: err.message || 'Error al cambiar el estado' });
  }
});
...
router.post('/:id/reset', protect, requireAdmin, async (req, res) => {
  try {
    ...
    if (!['in_progress', 'finished'].includes(torneo.status)) {
      return res.status(400).json({ message: 'Solo se puede reiniciar un torneo ya iniciado' });
    }
    if (torneo.participants.length < 2) {
      return res.status(400).json({ message: 'Se necesitan al menos 2 participantes' });
    }
    ...
    res.status(500).json({ message: err.message || 'Error al reiniciar el torneo' });
  }
});
...
router.post('/:id/matches/:matchId/result', protect, requireAdmin, async (req, res) => {
  try {
    const torneo = await Torneo.findById(req.params.id);
    if (!torneo) return res.status(404).json({ message: 'Torneo no encontrado' });
    if (torneo.status !== 'in_progress') {
    ...
    const match = await TorneoMatch.findOne({ _id: req.params.matchId, torneo: torneo._id });
    if (!match) return res.status(404).json({ message: 'Partido no encontrado' });
    if (match.status === 'bye') return res.status(400).json({ message: 'Es un bye, no se carga resultado' });
    if (!match.playerA || !match.playerB) {
      return res.status(400).json({ message: 'Aún faltan jugadores en este partido' });
    }
    ...
    if (draw) {
      if (torneo.format !== 'league') {
        return res.status(400).json({ message: 'Solo se permite empate en formato liga' });
      }
      match.winner = null;
      match.isDraw = true;
    } else {
      if (!winnerId) return res.status(400).json({ message: 'Falta winnerId' });
    ...
    if (torneo.format === 'single_elim' && match.nextMatch && match.winner) {
      ...
      const loserId = String(match.playerA) === winnerId ? String(match.playerB) : String(match.playerA);
      ...
      if (!isFinal) {
        await saveNotification(winnerId, 'tournament_advanced', {
          torneoId: String(torneo._id),
          torneoTitle: torneo.title,
          round: match.round,
        });
        io?.to(`user:${winnerId}`).emit('torneo:advanced', {
          torneoId: String(torneo._id),
          torneoTitle: torneo.title,
          round: match.round,
          timestamp: new Date().toISOString(),
      ...
      if (loserId && loserId !== 'null') {
        await saveNotification(loserId, 'tournament_eliminated', {
          torneoId: String(torneo._id),
          torneoTitle: torneo.title,
          round: match.round,
        });
        io?.to(`user:${loserId}`).emit('torneo:eliminated', {
          torneoId: String(torneo._id),
          torneoTitle: torneo.title,
          round: match.round,
          timestamp: new Date().toISOString(),
...
router.delete('/:id/matches/:matchId/result', protect, requireAdmin, async (req, res) => {
  try {
    const torneo = await Torneo.findById(req.params.id);
    if (!torneo) return res.status(404).json({ message: 'Torneo no encontrado' });
    if (torneo.status !== 'in_progress') {
    ...
    if (match.status !== 'completed') {
      return res.status(400).json({ message: 'Este partido no tiene resultado cargado' });
    }
...
router.get('/:id/groups', protect, requireAdmin, async (req, res) => {
  try {
    ...
    const totalPhases = await TorneoGroup.distinct('phase', { torneo: torneo._id });
...
router.post('/:id/games/:gameId/result', protect, requireAdmin, async (req, res) => {
  try {
    const torneo = await Torneo.findById(req.params.id);
    if (!torneo) return res.status(404).json({ message: 'Torneo no encontrado' });
    if (torneo.format !== 'groups') {
      return res.status(400).json({ message: 'Este endpoint es solo para torneos por grupos' });
    }
    if (torneo.status !== 'in_progress') {
      return res.status(400).json({ message: 'El torneo no está en curso' });
    }

    const game = await TorneoGame.findOne({ _id: req.params.gameId, torneo: torneo._id });
    ...
    for (const r of results) {
      ...
      if (!validPlayerIds.has(playerId)) {
        return res.status(400).json({ message: 'Un resultado contiene un jugador que no es del grupo' });
      }
      if (playerIdsSeen.has(playerId)) {
        return res.status(400).json({ message: 'Hay jugadores duplicados en los resultados' });
      }
      playerIdsSeen.add(playerId);
      const score = Number(r.score);
      if (!Number.isFinite(score)) {
        return res.status(400).json({ message: 'Todos los scores deben ser números válidos' });
      }
      parsed.push({ player: playerId, score });
    }
    if (parsed.length !== group.players.length) {
      return res.status(400).json({ message: 'Hay que cargar el score de todos los jugadores del grupo' });
    }
...
router.delete('/:id/games/:gameId/result', protect, requireAdmin, async (req, res) => {
  try {
    const torneo = await Torneo.findById(req.params.id);
    if (!torneo) return res.status(404).json({ message: 'Torneo no encontrado' });
    if (torneo.format !== 'groups') {
    ...
    const nextPhaseExists = await TorneoGroup.exists({ torneo: torneo._id, phase: { $gt: group.phase } });
    if (nextPhaseExists) {
      return res.status(400).json({ message: 'No se puede deshacer: ya se generó la siguiente fase' });
    }
    ...
    res.json({ message: 'Resultado deshecho', gameId: game._id, groupId: group._id });
  } catch (err) {
    res.status(500).json({ message: 'Error al deshacer el resultado' });
  }
});
...
router.patch('/:id/groups/:groupId/advanced', protect, requireAdmin, async (req, res) => {
  try {
    const torneo = await Torneo.findById(req.params.id);
    if (!torneo) return res.status(404).json({ message: 'Torneo no encontrado' });
    if (torneo.format !== 'groups') {
    ...
    const group = await TorneoGroup.findOne({ _id: req.params.groupId, torneo: torneo._id });
    if (!group) return res.status(404).json({ message: 'Grupo no encontrado' });
    if (group.status !== 'completed') {
    ...
    const nextPhaseExists = await TorneoGroup.exists({ torneo: torneo._id, phase: { $gt: group.phase } });
    if (nextPhaseExists) {
      return res.status(400).json({ message: 'No se pueden editar los promovidos: ya se generó la siguiente fase' });
    }

    const { advancedPlayers } = req.body;
    if (!Array.isArray(advancedPlayers)) {
      return res.status(400).json({ message: 'advancedPlayers debe ser un array' });
    }

    const playerSet = new Set(group.players.map((id) => String(id)));
    for (const id of advancedPlayers) {
      if (!playerSet.has(String(id))) {
        return res.status(400).json({ message: 'Hay un usuario que no pertenece al grupo' });
      }
    }
...
router.post('/:id/next-phase', protect, requireAdmin, async (req, res) => {
  try {
    ...
    const groups = await TorneoGroup.find({ torneo: torneo._id, phase });
    if (groups.length === 0) {
      return res.status(400).json({ message: 'No hay grupos en la fase actual' });
    }
    if (groups.some((g) => g.status !== 'completed')) {
      return res.status(400).json({ message: 'Hay grupos sin terminar en la fase actual' });
    }

    const advancers = [];
    for (const g of groups) {
      if (!g.advancedPlayers || g.advancedPlayers.length === 0) {
        return res.status(400).json({ message: `El grupo de la mesa #${g.tableNumber} no tiene promovidos` });
      }
    ...
    const requestedTableSize = req.body?.tableSize ? clamp(parseInt(req.body.tableSize), 2, 12) : null;
    const validation = validateNextPhase(advancers.length, requestedTableSize ?? torneo.tableSize);

    let tableSizeToUse;
    if (validation.valid) {
      tableSizeToUse = validation.isFinal ? advancers.length : (requestedTableSize ?? torneo.tableSize);
      ...
      return res.status(400).json({
        message: validation.warnings.join(' ') || 'No es posible armar la siguiente fase con la configuración actual',
        suggestions: validation.suggestions,
      });
    }

    await buildAndInsertGroupsForPhase(torneo, phase + 1, advancers, tableSizeToUse);
    ...
    for (const advUser of advancers) {
      await saveNotification(advUser, 'tournament_advanced', {
        torneoId: String(torneo._id),
        torneoTitle: torneo.title,
        round: phase,
        isPhase: true,
    ...
    res.json({ message: 'Fase siguiente generada', currentPhase: torneo.currentPhase });
  } catch (err) {
    res.status(500).json({ message: err.message || 'Error al generar la siguiente fase' });
  }
});

// GET /api/torneos/:id/next-phase/preview — admin previews how next phase would lay out
router.get('/:id/next-phase/preview', protect, requireAdmin, async (req, res) => {
  try {
    const torneo = await Torneo.findById(req.params.id).lean();
    if (!torneo) return res.status(404).json({ message: 'Torneo no encontrado' });
    if (torneo.format !== 'groups') return res.json({ valid: false, warnings: ['No aplica'] });
    const groups = await TorneoGroup.find({ torneo: torneo._id, phase: torneo.currentPhase });
    const advancers = groups.flatMap((g) => g.advancedPlayers || []);
    const validation = validateNextPhase(advancers.length, torneo.tableSize);
    res.json({ advancersCount: advancers.length, defaultTableSize: torneo.tableSize, ...validation });
  } catch (err) {
...
async function getFinalRound(torneoId) {
  const last = await TorneoMatch.findOne({ torneo: torneoId }).sort({ round: -1 }).select('round').lean();
  return last?.round || 0;
}
