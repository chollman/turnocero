...
router.post('/:id/request', protect, async (req, res) => {
  try {
    ...
    if (targetId === myId) {
      return res.status(400).json({ message: 'No podés enviarte una solicitud a vos mismo' });
    }
...
router.post('/:id/accept', protect, async (req, res) => {
  try {
    ...
    if (!fromUser) return res.status(404).json({ message: 'Usuario no encontrado' });

    const reqIndex = me.friendRequests.findIndex((r) => r.from.toString() === fromId);
    if (reqIndex === -1) {
      return res.status(400).json({ message: 'No hay solicitud de este usuario' });
    }
    ...
    const io = req.app.get('io');
    io.to(`user:${fromId}`).emit('friend:accepted', {
      fromUserId: myId,
      fromUsername: me.username,
    });
...
router.delete('/:id/request', protect, async (req, res) => {
  try {
    ...
    const reqIndex = target.friendRequests.findIndex((r) => r.from.toString() === myId);
    if (reqIndex === -1) {
