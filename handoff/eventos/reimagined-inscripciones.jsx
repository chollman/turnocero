    ...
    <div className="inscItem">
          ...
          >
            ...
            <div className="comprobanteInfo">
              <div className="comprobanteName">
                {reg.comprobanteType === "pdf"
                  ? "comprobante.pdf"
                  : "transferencia.jpg"}
              </div>
              <div className="comprobanteOpen">
                Abrir comprobante <Icon.External />
              </div>
            </div>
          </a>
      ...
      {status === "rejected" && (
        <>
          ...
          <button className="inscBtnUndo" onClick={onUndo}>
            ↺ Revertir decisión
          </button>
        </>
      )}
  ...
  return (
    <div className={`triageCol ${colorClass}`}>
      <div className="triageColHead">
        <span className={`triageColLabel ${labelClass}`}>◆ {title}</span>
...
function InscripcionesScreen({ eventId, onBack }) {
  ...
  const baseEvento = useMemo(
    () =>
      MOCK_EVENTS.find((e) => e._id === eventId && e.pendingRegs) ||
      MOCK_EVENTS.find((e) => e.pendingRegs) ||
      MOCK_EVENTS[0],
    [eventId],
  );
  ...
  const [confirmed, setConfirmed] = useState(
    (baseEvento.confirmedRegs || []).map((r) => ({
      ...r,
      handle:
        r.handle || r.name.toLowerCase().replace(/\s/g, "").replace(".", ""),
    })),
  );
  ...
  const reject = useCallback((reg) => {
    ...
    setRejected((r) => [
      ...r,
      {
        ...
        notes: "Rechazada desde la bandeja",
      },
    ]);
  }, []);
      ...
      <div className="inscHeader">
        ...
        <div className="inscStatRow">
          <div className="inscStat">
            ...
            <span className="inscStatValue inscStatValueOrange">
              {pending.length}
            </span>
          </div>
          <div className="inscStat">
            <span className="inscStatLabel">Confirmadas</span>
            <span className="inscStatValue inscStatValueGreen">
              {confirmed.length}
            </span>
          </div>
          <div className="inscStat">
