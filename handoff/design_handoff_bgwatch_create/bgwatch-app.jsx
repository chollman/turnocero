...
const PLAYS = [
  ...
  {
    id: 'pl3', date: '2026-05-12', game: 'Sky Team', initial: 'SK',
...
function PlayCard({ play, onOpen }) {
  ...
  const winSym = play.outcome === 'win' ? '✦ Ganaste' : '○ Perdiste';
  return (
    <div className="play" onClick={onOpen}>
      ...
      <div className="playInfo">
        ...
        <div className="playPlayers">
          {play.players.slice(0, 4).map((p, i) => (
            <Avatar key={i} user={p} size="xs" />
          ))}
          {play.players.length > 4 && <span className="playMore">+{play.players.length - 4}</span>}
        </div>
      </div>
