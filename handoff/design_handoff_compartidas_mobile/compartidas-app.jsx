...
function CompartidasApp() {
  ...
  const openPost = (id) => { setPostId(id); setScreen('permalink'); };
  ...
  return (
    <div className="app-root">
      ...
      <div className="switcher">
        ...
        <div className="switcherTabs">
          ...
          <button
            ...
            onClick={() => { setPostId(postId || MOCK_POSTS[0]._id); setScreen('permalink'); }}
          >
            Permalink
          </button>
        </div>
      ...
      {screen === 'permalink' && (
        <PermalinkScreen postId={postId} onBack={goFeed} onLightbox={setLightbox} />
      )}

      {/* Lightbox */}
      {lightbox && (
        <div className="lightbox" onClick={() => setLightbox(null)}>
          <button className="lightboxClose" onClick={() => setLightbox(null)}><Icon.Close /></button>
          <div className="polaroid" style={{ transform: 'rotate(-1deg)', maxWidth: '90%', maxHeight: '90vh' }}>
            <div className="polaroidPhoto polaroidFallback polaroidPhotoLandscape" style={{ width: 'min(700px, 80vw)' }}>
              {lightbox.caption || 'foto'}
            </div>
            {lightbox.caption && <span className="polaroidCaption">{lightbox.caption}</span>}
          </div>
        </div>
      )}
      ...
      <TweaksPanel title="Tweaks">
        ...
        <div style={{ display: 'flex', gap: 6, marginTop: 4 }}>
          <button onClick={() => setScreen('feed')} style={btnStyle(screen === 'feed')}>Feed</button>
          <button
            onClick={() => { setPostId(MOCK_POSTS[0]._id); setScreen('permalink'); }}
