...
- **`bggAggregations.js`** — `computeGameStats`, `computePlayedGames`, `computeTopPlayedGame` (aggregations sobre `BggPlay`, NUNCA sobre `BggCollection` — [[bgg-prefer-plays-aggregation]]).
...
- **`bggCooldown.js`** — `getManualRefreshRemainingMs`, `stampManualRefresh` para el cooldown del botón "↻ Actualizar" (60s); respeta [[bgg-username-case]].
...
- **`bggSyncEngine.js`** — `reconcileFull`, `probe`, `triggerBackgroundProbe`, `triggerBackgroundReconcile`, `stampProbeOutcome`, `stampReconcileResult`. Ver [[bgg-sync-engine]] y [[user-lock-semantics]] — las invariantes siguen vivas en el service.
- **`bggMutations.js`** — `buildPlayForm`, `validatePlayBody`, `submitToGeekplay`, `verifyPlayOnBgg`, `upsertPlayFromBgg`. **DELETE en `geekplay.php` queda inline en el handler** (`finalize=1` + `B1=Yes` arman un form ad-hoc distinto — [[bgg-write-quirks]]).
...
- **El handler HTTP no hace `try/catch`** si el router está migrado a [[asyncHandler-errorHandler-pattern]] — `throw httpError(status, msg)` y el middleware central responde.
