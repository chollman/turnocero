...
- **REINCIDENCIA 2026-05-22 — `EventoLudoteca.handleAdded`**: mismo patrón otra vez, esta vez en feature nueva. El POST `/eventos/:id/ludoteca` emite `evento:ludoteca-changed` al room antes del `res.json`. El parent `EventoDetail` dedupea en `onLudotecaChanged` por `_id`, pero el child `handleAdded` hacía `setItems(prev => [...prev, newItem])` sin checkear. Socket gana → parent state propaga al child via `useEffect(itemsProp)` → `handleAdded` agrega de nuevo. Resultado: card duplicada hasta recargar. Fix: dedup por `_id` en `handleAdded` igual que en el listener. **Patrón a internalizar**: si tu feature tiene **(socket listener que appendea a un array) + (handler optimistic que también appendea)**, ambos lados deben checkear `list.some(it => it._id === x._id) ? list : [...list, x]`. Es literalmente el mismo bug que `Eventos.handleCreate`. Cuando agregues una feature con esta forma, copiá el dedup de un lado al otro de entrada — no esperes a verlo duplicado en prod.
