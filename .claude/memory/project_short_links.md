...
Capa de short links sobre el mismo dominio (`<origin>/s/<code>`) para los deeplinks que se comparten por redes. Rama `feature/short-links` (2026-06-09), **no commiteada** al cierre. Pedido del usuario: "¿podemos crear short links…?". Decisiones tomadas con él: alcance = TODOS los compartibles (Compartidas, Eventos, Noticias, perfiles BG Watch); mismo dominio `/s/:code` (cero infra); aplicación **automática y transparente**.

**Por qué endpoint genérico get-or-create (no un campo por modelo):** trata los 4 tipos igual, incluido BG Watch que **no tiene documento** (el perfil se deriva del bggUsername). Creación perezosa, dedup por índice unique `{type, ref}`, sin migración.
...
- 6 call-sites: CompartidaCard, ResenaCard, NoticiaDetail (eager), EventoDetail.handleShare, CreatePlay.runShare. BG Watch NO tiene botón de compartir hoy → no se inventó UI (el backend igual soporta `bgwatch`).
