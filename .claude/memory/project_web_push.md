...
- **Iconos de la notif** (`client/src/sw/pushNotification.js`): `icon` = `/pwa-192x192.png` (logo "TO" a color, lado grande). `badge` = `/badge-96x96.png` (silueta "TO" BLANCA sobre transparente, generada con `sharp` desde `logo.svg`). El badge DEBE ser monocromo transparente: Android lo enmascara por alfa → un PNG opaco (como el viejo `pwa-64x64.png`) sale como cuadrado blanco en la barra de estado. Si se cambia la marca, regenerar el badge igual (blanco, transparente, ~96px).
