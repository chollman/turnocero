...
- `client/src/pages/bg-watch/buildPlayResult.js` — arma el snapshot desde los MISMOS `scorecardRows` que el preview en vivo (descarta `you`/`leader`/`key`, `score`→string, **sin location**, null si no hay juego/filas). Se hoisteó `scorecardRows` arriba de `submit` para reusarlo. Guarda `name`/`username` como snapshot/fallback; **no** congela la identidad (ver "Identidad EN VIVO" abajo) — el cliente NO manda `userId`.
- `client/src/pages/bg-watch/playResultToScorecard.js` — adapter snapshot→props del Scorecard: recomputa `leader`, `you=false`, `publicView:true`. Arma el `userMap` (clave = @BGG en minúscula) desde los `userId` **poblados** y usa `getUserDisplay(userId).name` para la fila; los jugadores sin cuenta caen al nombre de snapshot + avatar por iniciales.
...
- Server: `Compartida.playResult` (sub-schema `playResultSnapshotSchema` con `mode` enum + players acotados; reseñas lo nulean en `pre('validate')`). `routes/compartidas.js#sanitizePlayResult` coerciona/acota (team A-D, ≤24 players) y re-resuelve el thumbnail del juego desde `gameId` (best-effort). **El `playResult` cuenta como contenido** → una juntada solo-scorecard (sin título/texto/foto) es válida (gate relajado).

**Identidad EN VIVO de los jugadores (snapshot → referencia, 2026-06-09):** cada `playResultPlayerSchema` tiene `userId` (ref `User`, default null). El **server lo deriva** del @BGG en `sanitizePlayResult` (un único `User.find({bggUsername:{$in}}).collation(strength:2)` batcheado) — **no** se confía en un `userId` del cliente (sería spoofeable), igual que con el arte del juego. `populateCompartida` agrega `.populate("playResult.players.userId", "username displayName avatar")` (proyección acotada = la garantía anti-leak, porque el feed serializa con `toObject`, no `toJSON`). El `<Scorecard>` resuelve nombre+avatar del perfil **actual**, así editar nombre/apellido/avatar se refleja en juntadas viejas (única fuente de verdad). `name`/`username` quedan como fallback (anónimos, no-miembros, cuentas borradas). **No hay otro path** que cree compartidas con `playResult` (todo pasa por `POST /api/compartidas`). Backfill idempotente para juntadas previas: [server/scripts/backfill-playresult-userid.js](../../server/scripts/backfill-playresult-userid.js) (corrido en prod 2026-06-09). Reemplazó el enfoque previo (efímero) de congelar el nombre de TurnoCero en el snapshot vía `buildPlayResult`.

  - **Links en el scorecard (2026-06-09):** en `publicView` el adapter agrega por fila `profileHref` (`/usuarios/:id`, solo si es usuario TC) y `bgwatchHref` (`/bg-watch/:@BGG`, cualquier jugador con handle, no anónimo). El `<Scorecard>` (prop nueva `bgwatchEnabled`, default false, gateada por el caller con `isSectionEnabled("bgwatch")`) vuelve el **nombre** un link al perfil público y agrega un **ícono-link a BG Watch** (mismo glifo dado/grilla que `AuthorBgWatchLink` de CompartidaCard). `stopProp` en ambos (no burbujear al card). El nombre mostrado sale de `getUserDisplay(userId).name` = displayName||username (el populate manda `username displayName avatar`, **sin** nombre/apellido — consistente con el header del autor y privacidad; un usuario sin displayName se ve por su username, ej. "Blackwatch2689" — si quiere su nombre, lo setea en /perfil y se refleja en vivo).

  - **Gotcha de entorno (scripts que pegan a Atlas en esta máquina):** c-ares (resolver de Node) toma `127.0.0.1` como DNS y **rechaza las consultas SRV** de `mongodb+srv://` (`querySrv ECONNREFUSED`). PowerShell resuelve bien (usa el resolver del sistema). Fix: `require("dns").setServers(["8.8.8.8","1.1.1.1"])` al tope del script + correr con sandbox deshabilitado (red). Vale para cualquier script one-off contra la DB.
