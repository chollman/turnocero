...
**Agregar un campo nuevo a un modelo L2 inmutable (sin TTL) → backfill lazy (2026-06):** como `BggGame` NO tiene TTL (data inmutable), los docs ya cacheados nunca se refrescarían, así que un campo nuevo (ej. `playingTime`) quedaría `null` para siempre en ellos. Patrón: en `resolveXxx`, si el doc cacheado **no tiene la key** (lean → `undefined`, porque se persistió antes de agregar el campo), tratarlo como stale y re-fetchear a BGG para poblarlo. Los docs nuevos sin valor declarado tienen el campo en `null` (no `undefined`) y NO re-fetchean. La distinción `undefined` (doc viejo) vs `null` (sin valor) es la clave — `lean()` no aplica defaults del schema, así que el campo ausente queda `undefined`. Aplicado en `resolveGame` para `playingTime` (tiempo de caja de BGG, que alimenta la sugerencia de duración del PlayForm). BGG expone `playingtime`/`minplaytime`/`maxplaytime` (declarado del editor), **no** un promedio real de partidas logueadas.
