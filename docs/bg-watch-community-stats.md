...
1. ~~**Scoping real por Comunidad** (fase 2).~~ **HECHO.** Los routes del hub corren `optionalAuth → resolveCommunities → attachBggScope`, que deriva `req.bggScope` = `communityMemberUsernames(req.viewingCommunities)` (lowercase, dedup) y lo pasa como `bggUsernames` a cada aggregation. La función devuelve `null` (= global, sin filtro) cuando el scope incluye la base — todos pertenecen a la base, así que filtrar sería un no-op + un `$in` gigante; el modo tenant / "ver solo una sub-comunidad" sí acota. Una comunidad real sin miembros conectados → `[]` → hub vacío (no cae a global). `gameOwners` siempre necesita allowlist: en modo global usa `connectedMemberUsernames()`. `companeros` y `h2h` quedan SIN scope a propósito (derivan de las partidas propias de usuarios nombrados, no del subconjunto de miembros). Tests: 4 unit + 4 integration.

   **Cliente:** el scope sigue la selección del `CommunitySwitcher` (persistida en `communityPrefs.viewing`, leída por `resolveCommunities` en cada request). El re-fetch en vivo ya lo da el mecanismo existente `viewingVersion → routesKey` (App.jsx remonta `<Routes>` al cambiar el viewing) — NO agregar deps de `viewing` a los `useEffect` del hub (duplicaría el fetch). El hub muestra un **`ScopeBanner`** (en `BgWatchComunidad.jsx`) que aclara el scope: lee el contexto null-safe (`useContext(CommunityContext)`), se oculta sin provider / con ≤1 comunidad / en modo tenant, y distingue **global** (la base está tildada ⇒ "toda la comunidad" + pista para destildar TurnoCero) vs **acotado** (nombres de las comunidades elegidas). +5 client tests. Gotcha de diseño que confunde a usuarios: como la base "TurnoCero" ES todos, mientras esté tildada el hub es global; para acotar hay que dejar tildada solo una sub-comunidad.
